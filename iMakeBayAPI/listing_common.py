...
# ===================================================================
# 仕入元が「1枚ではない」と書いているか (2026-08-23 / 2026-09-04 共通化)
# ===================================================================
# ★ここに移した理由 (2026-09-04): 元は iMakTCG/psa_to_csv.py にあり、
#   **出品する対象**にしか当てていなかった。同じ判定を **仕入候補**にも
#   当てたいが、psa_to_csv は重い (Selenium 等) ので候補側から import できない。
#   実測: 人が『違う』と外した候補249件のうち **34件がまとめ売り/連番**で、
#   機械で判るものを毎回 人に見せていた。
...
_SUPPLY_LOT = re.compile(
    ...
    # ★2026-09-04: カードは「枚」だが、一番くじ/フィギュアは「点・個・種」で数える。
    #   ただし **セットと書いてある時だけ** 拾う。「残り2点」(在庫表示) を
    #   まとめ売りと読まないため (単位だけでは出品枚数か在庫数か判らない)。
    r"|(?<![0-9])([2-9]|[1-9][0-9])\s*(点|個|種|本)\s*セット"
    # ★2026-09-17: 「〜、〜、テツノワダチexの3点」(カード名を並べて最後に数) がすり抜けて
    #   PSA 補URL③ の候補に出ていた。**「の N点」** の形だけ拾う (「残り2点」は _SUPPLY_LOT_NOT 側)。
    r"|の\s*([2-9]|[1-9][0-9])\s*(点|個|種))")
