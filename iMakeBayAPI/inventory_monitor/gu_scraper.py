"""gu_scraper - GU 商品ページの在庫・価格スクレイパー (独立モジュール).
...
API 経路 (Selenium 不要の理由):
  GU の商用 L2S API (UNIQLO と同 commerce/v5 API、FAST RETAILING 系列) が公開エンドポイント (認証不要) で在庫・価格を JSON 返却:
    https://www.gu-global.com/jp/api/commerce/v5/ja/products/{pid}/price-groups/{pg}/l2s?withPrices=true&withStocks=true
...
使用例:
    from gu_scraper import fetch_product_inventory
...
def _call_l2s_api(product_id: str, price_group: str = "00") -> dict:
    ...
    url = GU_API_TEMPLATE.format(product_id=product_id, price_group=price_group)
...
def fetch_product_inventory(
    ...
    for item in l2s_filtered:
        ...
        if status_code in ("IN_STOCK", "LOW_STOCK"):
            ...
            print(f"    ⚠️ gu_scraper: 未知 statusCode={status_code!r} "
                  f"l2Id={l2id} → ✕扱い (要コード対応)")
            in_stock = False
        # 販売停止中は在庫があっても ✕
        if not sales_active:
            print(f"    ⚠️ gu_scraper: sales=False l2Id={l2id} → ✕扱い (販売停止中)")
            in_stock = False
