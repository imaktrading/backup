...
**走行結果 (実測)**
- 検索語 162 / 見つけた出品 3,097 / 候補 1,131 / **シート 1,129** (2件は既存と重複)
- 10:10 → 17:27 (**7時間17分**)。空きメモリ 最低 3.6GB (13:30 の監視くんの巡回と重なった時)、BSOD 無し
- 番号だけの語は和名の約半分: 番号語 平均15.1件 (114語中16語が0件) / 和名語 平均28.5件 (0件なし)
- **未判定 (取得失敗) 220件**。URL は `debug/mercari_treasure_20260920T101007.json` の
  `failed_urls`。拾い直しは HQ 判断待ち (要対応)。捨てた内訳: 出品者評価不足510 /
  PSA10でない397 / 番号が読めない413 / まとめ売り241 / 押さえ済65

**直した不具合 2件 (走行中に発覚)**
1. **収集が最後まで保存されなかった**: 162語の収集に1時間半かかるのに、終わるまで保存しない
   作りだった (途中で落ちると全部消える。9/13 の UT 140件消失と同じ型)。
   `collect_multi_keyword_urls` に `on_keyword_done` / `resume_urls` / `resume_by_keyword` を足し、
   **1語ごとに保存**、`--resume-from-json` で続きから再開。実際に3語で止めて再開し、
   4語目から検索が始まる所まで確認した。既存の psa10 走行の挙動は変えていない (追加のみ)
2. **`SKIP_UNREADABLE` の定義漏れ (9/8 の `0523ea3a` から)**: 参照だけ入って定義が無く、
   鑑定番号が読めない出品に当たるたび NameError → `item_error`。落とす結果は意図どおりだったが、
   3連続でブラウザが作り直される作りだった。pyflakes で全対象を洗い、**未定義名は他に無い**ことを確認。
   9/8 user 確定どおり `True` を定義

**失敗と反省**
- 最初の走行は、止めた `--dry-run` の Chrome が残ったまま本番を撃ち、最初の語が0件になって
  セッション切れで落ちた (同じプロファイルを2本同時に走らせない)。落ちたのに55分気づかず報告が遅れた
- 通知を絞りすぎて (500件ごと) 30分間ゼロ、ユーザーへの報告が4時間空いた
- どちらも skill `treasure-hunt-harvest` に「守ること」として書いた

**skill**: `treasure-hunt-harvest` を新設 (走らせ方 / 所要 / 保存と再開 / 実測 / 守ること)。
`harvest-targeting` のトレジャーハント節は案内1行に置き換えた。

## 2026-09-18 — BSOD 対策 (Chrome 残留kill + GPU切り) + トレジャーハント収集 新設

PC が 9/16 から1日2〜4回 BSOD。HQ から3本の依頼が来て全部対応した。

**① 残留 chrome/chromedriver を必ず落とす (`74006f1`)**
- 調査結果: 並列は元から無い (1走行=Chrome1本、1タブ逐次。control_panel も
  `_running` フラグで多重起動を止めている) ので、**多重起動は原因ではない**
- 真因側: 走行10本のうち **6本 (psa10 / mercari_search / amazon_search /
  restock_psa10 / snkrdunk / snkrdunk_op) は 残留killを一切呼んでいなかった**。
  既にある4本も開始時だけで終了時は無い。さらに既存の `kill_chrome_for_profile` は
  chrome.exe しか見ておらず、**chromedriver.exe は誰も落としていなかった**
  (--user-data-dir を自分のコマンドラインに持たないため profile 絞りに掛からない)
- 対応: `kill_orphan_chromedriver()` を新設 (chromedriver.exe は全kill して安全)。
  6本に **開始時 + finally (異常終了時も通る)** で両方を呼ぶよう追加
- 効果: 9/18 の走行後、残留プロセス **0個** を実測

**② トレジャーハント収集を新設 (`bf30747`)**
- HQ の `demand_market.csv` (eBay 実売台帳から「2個以上売れ、かつ うちが出していない」
  114種) を検索語にする別走行。**既存の psa10 走行は触っていない**
- 語は 和名 + 番号の両方 (162語)。書込先は単一タブ `mercari_psa10_treasure`、
  末尾 AJ 列に **上限判定 (上限内/超過)**。★上限超過でも**落とさず印だけ付ける**
  (落とすと出品数が減るため。HQ の明示要件)
- 一覧が空なら何もしない。判定できない時 (番号が読めない/価格不明) は空欄にして、
  「超過」と混同しない
- ★実装中に pre-commit が拾った欠陥: 最初 `on_flush` を渡しておらず
  **最後にまとめて書く作り**になっていた (= 落ちたら全部消える。UT で140件失った型)。
  途中書込を入れて解消。守りのテストが効いた1件

**③ Chrome の GPU 描画を切る (`a045ac0`)**
- HQ の見立て: 非headless で起動 → ウィンドウを隠す → 隠しても描画を続けさせる
  (`CalculateNativeWinOcclusion` 無効化) = **見えないウィンドウに何時間も描画し続ける**。
  画面ドライバをこう叩くのはこのPCでは抽出くんだけで、bugcheck もドライバ系
  (0x0A / 0x1E / 0x3B)
- `--disable-gpu` を driver 生成 **5箇所すべて**に追加 (1箇所だけ直すと次の収集で漏れる)
- 実走 (17:23:49〜17:57:22 / 45語 / dry-run / 詳細3件):
  - **落ちず固まらず完走** (走行中の bugcheck 0件)
  - **件数は落ちていない** — 1,340 URL、1語あたり18〜35件 (従来記録の「1語15件で
    頭打ち」より多い)
  - メモリ: プロセス23→31 / **2.51GB→3.36GB** (HQ 目標の4GB以内)
  - ★取得失敗1件 (詳細3件中) あるので、この走行を「正常」とは書かない
- 判定: 1回33分では確定と言えない (BSOD は1日2〜4回の頻度)。②③④ (画像非読込 /
  driver 再生成 / メモリ上限) は フル走行で確かめてから

### 次のアクション

- **フル走行 (1〜2時間) を何本か**回して `--disable-gpu` の効果を確定させる。
  落ちるなら ②③④ へ、落ちないなら画面ドライバで確定
- トレジャーハントの **夜間 cron (03:00) が未登録**。HQ には 03:00 と回答済みだが
  タスク登録はしていない (システム側の登録なので指示待ち)
- `2026-09-16_ut_rows_without_photos.md` が未回答のまま残っている
- 9/18 14:05 / 16:39 の BSOD は Harvest の走行中ではなかった
  (該当時刻に収集は動いていない)。Inventory / Mercari 側の可能性を HQ に共有済

## 2026-09-14〜18 — UT サイズ/色/写真の是正 + PSA10 優先キャラ語追加

**UT サイズが取れていない (user 指摘) → 2つのバグを是正**
- TCG skip キーワード誤爆: `should_skip_color_size` は PSA10 等のカード出品向けの
  判定だが、UT の説明文の趣味アピール文 ("ポケカ好きの方も" 等) に混ざって服が
  誤って skip されていた (実測360件中2件)。タイトルが明らかに服なら skip しない
  よう是正 (`_APPAREL_TITLE_MARKERS`)
- サイズ構造化欄はカテゴリ依存: メルカリの「サイズ」欄はホーム>UNIQLO>ファッション>
  メンズ>トップス>Tシャツ で出品された時にしか無い。それ以外は出品者が説明文に
  自由記述しているだけ。`_extract_size_from_description()` を追加 (「サイズ: M」
  「Sサイズ」等に対応、手がかりが無ければ空欄のまま)。実測66件中52件を回収
- `tools/backfill_size_color_uniqlo_ut.py` で既存618行に反映
  (色618/618・サイズ584/618)。★実装中の事故: 空欄でない列まで無条件で
  再判定していたため Vision の判定揺れで既存の色4件を誤って上書き (ブラウン→
  ホワイト等) → 元に戻し、「空欄の列だけ埋める」よう修正済み

**HQ 依頼: メルカリ写真に他商品・出品者アイコンが混入 (618行中390行)**
- `_extract_image_urls()` のフォールバック selector がページ内の mercdn.net 画像を
  全部拾っていた。商品本体画像パターンのみへ絞り、一致0件でもフォールバックせず
  空を返す (fail-closed)。UT 1件 + PSA10 1件を実走確認、混入0件を確認して完了報告済み
  (`2026-09-14_ut_mercari_photos_include_other_items_response_done.md`)

**HQ 依頼: UT の写真URL列が0本の行が22件 → 描画待ちの抜け穴**
- カルーセルが説明文より遅れて描画されることがあり、一発 find だとタイミング負けして
  空を返していた (`_extract_description` (8/17) と同じ形の抜け穴)。出現を待ってから
  諦めるようポーリングを追加 (`IMAGE_WAIT_SEC=5.0`)。★22件中の残務:
  実際の再訪問バックフィルと HQ への回答は未実施 (次回続き)

**PSA10 検索キーワードに人気キャラの優先語を追加 (user 提供)**
- 海外 eBay 無在庫で取引ボリューム・PSA10相場が安定して高いキャラ (ポケモン20/
  ワンピース20) を Aランク優先語として `psa_search_terms.POPULAR_CHARACTERS` に追加。
  型番は検証していないため使わずキャラ名のみ。ゲームごとに 優先キャラ→弾コード→
  一般語 の順。`build_keywords()` の既定動作に乗るので runner 側の変更は不要

**UT 収集の実行結果**
- 9/13 走行の残り (中断分) を再実行、140件追加。中間スプシ `mercari_uniqlo_ut`
  は計618行、色/サイズともほぼ埋まった状態で出品くんの目視待ち
- `harvest-targeting` skill に今回の学び (サイズ構造化欄のカテゴリ依存 / TCG誤爆 /
  バックフィルツールの「空欄だけ埋める」注意点) を追記済み

**環境メモ**: 9/16〜9/18 にかけて PC が繰り返し異常再起動 (Windows Event ID 41、
Windows がメモリ診断を自動スケジュール)。Harvest の処理が原因ではなく実機で確認済み
(ハードウェア側の疑いを user に報告)。作業がたびたび中断されたが、都度 git 状態を
確認して再開した

## 2026-09-13 まとめ — UT をカタログ起点・アニメ漫画に寄せた

**決まったこと (user 確定)**
- UT の仕入先は **メルカリだけ**。仕入れが芳しくなければラクマを追加
- 価格帯 **500〜12,000円** (既定値に入れた)
- 検索語はカタログの **公式で買えない UT** (消えた商品も含む) から作る。アニメ・漫画だけで **47語**
- カタログは **検索語の元にだけ**使う。KEY 付け・画像照合は抽出ではやらない (目視で確定)

**分かったこと**
- 「消えた商品」を外していたせいで アニメ・漫画がほとんど見えていなかった。
  カタログも同じ見立て (大人の UT 2,227件のうち消えた572件に溜まっている)。
  カタログ側は同日に掘り直し済み (呪術 1→16 / 鬼滅 4→10 等)
- 画像で **探す**ことはできない (メルカリ・ラクマに画像検索の口が無い)。照合はできるが抽出では使わない
- 他サイトの件数 (ユニクロ UT Tシャツ): ヤフーフリマ 18,768 / ラクマ 新品販売中 3,000 /
  ヤフオク 新品即決 約800 / メルカリは総数を出さない

**走行結果 (19:16〜21:29)**: アニメ漫画 47語。検索 **1,019件** → **採用 360件 / スプシに360件書込済**
(途中保存が効いた: 5件ごと74回)。`mercari_uniqlo_ut` は全478行 (うち今回+POC の400行に X列あり)。
- 採用の多い語: ブルーロック20 / 名探偵コナン19 / ダンダダン18 / 劇場版ハイキュー18 / CAPCOM 40th 17 /
  NARUTO17 / SPY×FAMILY16 / 米津玄師15 / チェンソーマン15 / 推しの子14 / ゼルダ ティアキン13
- 0件の語: 進撃の巨人 第2弾 / マンガキュレーション UT
- タグ番号が読めた 130件 / 価格 中央値 ¥2,000 (¥599〜¥10,000)
- 落とした内訳: ユニクロのTでない237 / 新品でない61 / コラボと判定できず55 / セラー評価不足45 /
  本人確認なし28 / 既にスプシにある21

★**⚠️要対応: 途中で打ち切り**。ドライバが連続3件失敗 → 再起動を3回 (20:05 / 20:33 / 21:01) → 21:29 に中断。
**取得失敗33件 / 未処理 約180件** (1,019 − 処理済 840)。
失敗が **約28分おき**に規則的に起きており、ドライバの偶発ではなく **メルカリ側の制限** の疑い。
回し直せば スプシにある360件は開かずに飛ばすので、残りだけ取れる

**事故**: 前回の走行で 140件 keep 後にドライバが落ち、全部消えた (下の節)。直して再走中
...
- **仕入先はメルカリだけ** (user 確定)。仕入れが芳しくなければラクマを追加。
  ラクマ・ヤフオク・ヤフーフリマの実測は skill `harvest-targeting` に残した
- 検索語の条件を **公式から消えた商品も含む** に変更。アニメ・漫画の語 **47語**
  (38語は手打ちでは出てこない作品)。カタログからも同じ見立ての回答 (消えた572件に溜まっている)
- カタログは **検索語の元だけ**に使う。KEY 付け・画像照合は抽出ではやらない (目視で確定)

## 2026-08-22 — メルカリのユニクロUT収集 / **補URLは機械で束ねない** (user 確定)

- `run_harvest_mercari_uniqlo.py` 新設。UT+GUのコラボT・新品未使用のみ。**78件投入**
  (`mercari_uniqlo_ut`)。呪術13/ジブリ12/ポケモン10/鬼滅8/GU8/DB7/スヌーピー7/ディズニー7/
  ワンピース6。中央値 ¥1,999。画像78件・サイズ62件
- ★**服の「ワンピース」をアニメ ONE PIECE として拾う事故**を下見で発見し是正
  (`UNIQLO クルーネックTワンピース 半袖`)。日本語は字面が同じ。テストで固定
- ★**補URL (同一商品の別出品) は機械で束ねない。目視で埋める** (user 確定)。
  公式APIで商品ID (E4xxxxx) は引けるが、メルカリのタイトルは「ポケモン UT M」程度しか
  書かず、同IPで柄違いが何十種もある。**取り違えると売れた後に別商品を送ることになる**
  (SNAD → Defect Rate)。機械が確定できない物を束ねる実装自体を作らない

## 2026-08-21 (続) — HQ の3依頼に対応 (35時間 止めていた)

作業ウィンドウを開きっぱなしにしていたため、8/20 の HQ 依頼3本が入り口で止まっていた
(user 指摘)。まとめて対応し `2026-08-20_gacha_recollect_spec_response.md` で回答。

- **サンリオ / ぬいぐるみ系を収集時に落とす** (`is_excluded_category`)。
  CPSC は stuffed animals を **対象年齢の印字と無関係に**児童製品扱い。サンリオは
  2026-06-29 の user 判断。語はHQ指定に**追加**(サンリオ系キャラ名/ぬい活/もふもふ/クッション)
- **既存分を掃除: 116行削除** (除外区分82 / 台紙あり・なしの重複34)。控えは `debug/deleted_*.json`
- **I列に書くのをやめた** (出品くんの英語タイトル列だった)。271件クリア。
  公式は **V列=商品ページURL / W列=公式画像** に変更
- **公式の商品ページ・画像・対象年齢は「公式起点」の経路なら必ず取れる**
  (`run_harvest_gashapon_line.py`)。バンダイのみ。タカラトミーアーツ公式は JS 描画で不可

中間スプシ **279行** (auc_yuyou 88 / auc_toysanta 141 / mirakikaku 35 / mejirushi 12 / jugem2020 3)

## 2026-08-21 — トイサンタを主力に / 死んだURLを掴む事故を止めた

中間スプシ **260行** (バンダイ217 / タカラトミーアーツ31 / クオリア8 / キタンクラブ2 /
ブシロード2)。店別 auc-yuyou139 / auc-toysanta79 / mirakikaku42。**年齢確認済196行**。

### ★事故: 存在しない商品を32件入れていた (user 指摘)

jugem2020 は **検索に出るのに商品ページが全部404**で、開くと店トップへ転送される。
気づかず **店トップの文言を商品情報として拾って**いた (価格も配送予定も別物)。
`fetch_detail` に「開いた後に URL が変わっていたら捨てる」を入れ、32行は削除、店も外した。

### 送料は合算に変更 (user 指示)

送料無料縛りをやめ、配送情報欄の送料を読んで **M列 = 本体 + 送料**。H列に内訳。
読めない物は採らない。実測: 260行中86行が送料あり (330円のメール便 / 680円の宅配便)。

### 走行が空振りした原因 3つ (全部こちらの作り)

1. **名前解決の一時失敗で店の検索が丸ごと0件**になっていた → 3回リトライ
2. **トイサンタの配送予定2形式が読めず候補50件全滅** → `即納｜…当日出荷` と
   `最短8/22(翌日)お届け` を追加
3. **1店で枠を食い切り後続店を検索しないまま終了** → 店ごとの上限

### こけても途中まで残す (user 指示)

5件ごと保存 / 書込失敗は捨てずに持ち越し / 連続5件失敗でドライバ再起動 (2回まで) /
走行開始時に自分の profile を掴んだままの chrome を片付け。

## 2026-08-20 (続7) — テーマ別の候補数を測り直して記録 (user 指摘)

8/19 に調べたテーマ一覧を日報に1行の要約でしか残しておらず、**元のリストを失っていた**
(user 指摘)。14テーマで測り直して skill に表で残した。

5社に該当する候補 (auc-yuyou + mirakikaku / 重複除去): **合計269件**
ディズニー93 / 仮面ライダー70 / サンリオ36 / ポケモン35 / すみっコ9 / マリオ7 /
ドラえもん6 / スヌーピー6 / ワンピース3 / めじるし2 / ちいかわ・動物・ガンダム 0〜1

- **コードの4テーマ (サンリオ/めじるし/動物/お菓子) では39件しか拾えない**。
  ディズニー93・仮面ライダー70・ポケモン35 を丸ごと見逃していた
- ★仮面ライダー/プリキュア/戦隊は **対象年齢3才**が多く、年齢ゲートで落ちる (実測26件)。
  候補数 = 採用数ではない

### 楽天の店舗内検索は AND (2026-08-20 実測・前言訂正)

「検索語を無視して全件返す」は誤り。540件は **12ページ×45件 = こちらのページ上限**だった。
- でたらめな語でも 11〜37件返る (ヒットが少ないと関連商品で埋める)
- **検索対象はタイトルだけでなく商品説明も含む**。mirakikaku で「バンダイ」が当たるのは
  説明に `メーカー：バンダイ` と書いてあるため。先頭12件を実際に開いて **12件ともバンダイ**
- → 狙い撃ちモードを「タイトルにメーカー名がある物だけ」にしているのは **取りこぼし**。
  mirakikaku のバンダイ品が全部落ちている (未対応)

## 2026-08-20 (続6) — バンダイ狙い撃ちで100件 / 年齢ゲートが26件を弾いた

`--maker バンダイ --quota 100 --max-pages 12` で **100件追加**。中間スプシは **147行**
(バンダイ114 / タカラトミーアーツ24 / クオリア7 / キタンクラブ2)。全行に画像・説明・公式URL。

- ★**対象年齢3才 = 26件を自動除外**した (仮面ライダー / プリキュア / ゼンカイジャー /
  おかあさんといっしょ 等)。**バンダイのガチャの2割は児童向け**で、そのまま出していたら
  CPSIA 違反の出品になっていた。ゲートを入れた効果がそのまま出た形
- **113行が「対象年齢15才以上」を公式で確認済**。この分は出品時の目視が要らない
- 目視が要るのは残り34行 (タカラトミーアーツ/クオリア/キタンクラブ = 公式に年齢記載なし)

### テーマ検索では取りこぼしていた (実測)

5メーカーに該当する候補は検索だけで **約900件** ある (タカラトミーアーツ420 / バンダイ240 /
ブシロード258 / キタンクラブ12 / クオリア数件)。テーマ4本 (サンリオ/めじるし/動物/お菓子) では
**ブシロード258件が1件も出てこなかった**。5社に絞ったなら **メーカー名で引く方が素直**
→ `--maker` を新設。`kidsroom` はどの検索でも0件 (実質死んでいる)

## 2026-08-20 (続5) — 5メーカーで再収集 + バンダイ品は対象年齢を公式で自動確認

**中間スプシは 21行**に (対象外81行を削除 → 新規10行を追加 → 年齢NG1行を削除)。
内訳: タカラトミーアーツ12 / バンダイ7 / キタンクラブ1 / クオリア1。
**全行に 画像・商品説明・公式URL が入っている**。

- 再収集 (`--headless --max-pages 4`): 候補157件 → 即納12件。
  落とした内訳: メーカー対象外 138 (検索時) + 24 (詳細時) / 配送予定が読めない 121 / 予約90
- **バンダイ品は JAN で公式の対象年齢が取れる** (`gacha_age.py` 新設)。
  `gashapon.jp/products/detail.php?jan_code=<JAN>000` を引き、**15才未満と分かったら入れない**。
  読めない時は落とさず目視に回す (「読めない」と「15才未満」を混同しない)
- ★実際に1件捕まえた: **スーパーマリオ めじるしアクセサリー ヨッシーコレクション (¥29,800) は
  対象年齢 6才** = CPSIA の児童製品。公式ページで確認して削除済
  (控え: `debug/rakuten_gacha_removed_age6_20260820.json`)
- 対象年齢を書けた行は H列末尾に `対象年齢: 15才以上` を追記済 (バンダイ7件)

## 2026-08-20 (続4) — 収集対象を 5メーカーに絞る (user 確定)

**バンダイ / タカラトミーアーツ / クオリア / キタンクラブ / ブシロードクリエイティブ のみ**。
メーカーが判定できない物も採らない (fail-closed)。`gacha_maker.ALLOWED_MAKERS`。

- 落とす場所は 2箇所。①検索直後: タイトルでメーカーが分かって対象外なら**詳細を見に行かない**
  ②詳細取得後: 商品説明の「メーカー：」まで見て対象外なら捨てる (mirakikaku 対策)
- **既存93行にこの基準を当てると残るのは 12行** (タカラトミーアーツ10 / クオリア1 / キタンクラブ1)。
  内訳: ケイカンパニー9 / フクヤ9 / トイズスピリッツ9 / アイピーフォー7 / ケーツーステーション6 /
  ベネリック4 / 不明23 … が対象外
- 収集量は **1/8 程度に落ちる**見込み (8/19 実測でも auc-yuyou の120件中バンダイは0件)

## 2026-08-20 (続3) — 商品説明 (H列) と JAN を取る / 目視は続行 (user 確定)

- **user 判断: 対象年齢の目視はやる** → ガチャ収集は継続。メーカーでは絞らない
  (公式サイトがあっても年齢は載っていない。フクヤ実測: 画像・サイズ・発売月のみ。
  年齢が公式で取れるのは実質バンダイだけで、今回の93件にバンダイは0件)
- **H列 (商品説明) が空だった** → 楽天の `item_desc` から取得。**全何種の内訳とサイズは
  ここにしかない** (タイトルには入らない)。末尾の関連商品リンクは落とす
- **JAN が `itemprop="gtin13"` で確実に取れる** (92/93件)。H列末尾に `JAN: <番号>` を付けた。
  バンダイ品なら 出品くん が `gashapon.jp/products/detail.php?jan_code=<JAN>000` で年齢を引ける
- **mirakikaku は説明の `メーカー：<名前>` 欄にメーカーを書いている** → I列の公式URLが
  **61 → 70/93** に増えた
- 既存93行に反映済 (説明93 / 公式URL70 / 画像93)

## 2026-08-20 (続) — 楽天の商品画像が1枚も取れていなかった (user 指摘で発覚)

**93件中 93件が「商品写真0枚」**だった。集めていた平均8枚は全部 **店のヘッダ/サイドのバナー**
(`cabinet/parts/header/menu0.jpg` `rkanban.jpg` 等)。原因は `image.rakuten.co.jp` の URL を
**出現順に8枚**取っていたこと。楽天の店舗テンプレはバナーが先に並ぶ。

直し方 (`rakuten_item.extract_images`): 商品の物と確定できる 2つだけ入れる。
① `og:image` ② **ファイル名に商品コードを含む画像** (`g260736s02t.jpg` / `2608001_c0.jpg`)。
どちらも無ければ **空** (バナーで埋めない)。実測7件で全件1枚以上取得、他商品の写真は混入なし。

★HQ には 8/19 に「平均8枚で足ります」と伝えていたが、**その8枚は商品写真ではなかった**。
既存93行の画像 (G列) は入れ替えが必要。

## 2026-08-20 — ガチャの I列に メーカー公式URL (user 依頼)

`rakuten_gacha` の **I列 = メーカー公式サイトURL**。既存93行も埋め済 (**61行に URL / 32行空欄**)。

- 判定はタイトルの「…セット **<メーカー>** ガチャポン…」スロットのみ (`gacha_maker.py` 新設)。
  実測で決めた理由: 楽天の商品ページに **メーカー欄が無い** (10/10件) / 全文検索すると
  **同じ店の「おすすめ商品」欄**を拾う (16件全部が `Qualia` に一致した)。スロットは 81/93 で取れる
- 表に無いメーカー / スロット無しは **空欄** (推測で URL を入れない)
- URL は user 提供の15社 + 楽天に多く出た4社 (ケイカンパニー / フクヤ / ケーツーステーション /
  ベネリック。実地確認済)。**最多の ご当地本舗夢屋 (13件) は公式サイトが無く SNS のみ** → 空欄
- 残りの空欄: 夢屋13 / スロット無し (mirakikaku) 12 / ディーアイエス2 / ウルトラニュープランニング2 /
  ピーナッツクラブ・エスケイジャパン・レインボー 各1
- **HIGH の I は PSA cert 列**なので、コピー時に I を持ち込まないよう HQ に連絡済
  (`hq/requests/2026-08-20_gacha_official_url_col_i.md`)

## 2026-08-19 (続2) — 楽天ガチャポン収集を新設 (user 依頼・初回93件投入)

### やったこと

3ショップ (auc-yuyou / kidsroom / mirakikaku) から **ガチャポンのコンプ品・即納のみ**を集める
仕組みを新設し、中間スプシ `rakuten_gacha` に **93行**投入。ルールは skill
`rakuten-gacha-harvest` に集約。

判定は安い順: ①タイトル (コンプ品か / 予約か / おもちゃか) → ②本番の仕入元と重複 →
③ブラウザで配送予定を読む (1件6秒)。**発送日が読めた物だけ採用**、読めなければ入れない。

### 実測でひっくり返ったこと

- **在庫マーク (availability) は使えない**。予約品も `InStock` を返す (21/21件)。
  予約品もカートに入るので当然で、「買えるか」ではなく「いつ発送されるか」を見る必要がある
- **配送予定は JS 描画後にしか出ない**。検索は HTTP のみで済むが、ここだけブラウザが要る
- 楽天の商品ページに **JAN はある (12/12件) が対象年齢は無い (14件中0件)**

### user 指摘で直したもの

1. **送料込みで見る必要がある** → 検索を送料無料 (`&f=2`) に限定 = 表示価格が総額。
   ★初回100件は絞る前に集めており **43件が送料有料**だった → 44行削除して入替
2. **実際の食べ物は扱えない** → おもちゃ語がある物だけ通す fail-closed ゲート
   (「お菓子のミニチュア」は可、実食品は不可)
3. **対象年齢 (CPSC)** → 収集側では機械判定できないため、**出品時に画像で目視**する分担に
   (user 確定)。HQ に連絡書 `2026-08-19_gacha_age_check_handoff.md` を提出

### 調べて分かった事情

- 国内シェアは **バンダイ + タカラトミーアーツで6割超** (2024年度 約1,410億円)。
  次点が キタンクラブ / クオリア / ブシロード / スタンドストーンズ (各20〜40億円)
- **対象年齢を公式で確認できるのはバンダイのみ** (gashapon.jp は JAN 直引き可)。
  タカラトミーアーツ公式は対象年齢もJANも載せていない
- 「めじるし」は中小が強い (auc-yuyou 120件中 バンダイ0 / タカラトミーアーツ20)
- テーマ別の供給 (3店・コンプ品): サンリオ499 / 猫動物339 / めじるし207 / スイーツ158 …
  ちいかわは9件と薄い

### HQ とのやり取り

- `2026-08-19_rakuten_gacha_fields.md` → 回答: **列は PSA10 と同じで足りる**。
  R列は `カプセルトイ` (表記ゆれ禁止)。**N列には書くな** (数式)。初回50〜100件、一気に入れない
- `2026-08-19_n_column_conflict.md` → 回答: HIGH の **N は ARRAYFORMULA の spill**。
  セル単位で見ると空に見えるので「値」と誤読していた (こちらの実測ミス)。
  **中間スプシの931行はそのままで可**、HIGH へコピーする時に N と P を含めなければよい。
  実機確認で HIGH の N はリテラル値0件 / P は2,238行すべて数式 = **既存コピーは無傷**

### 残務

- メーカーを絞るかは **出品くん側の目視方法を確認してから**決める (user 保留)
- PSA10: 再取得も失敗した111件 / 目視待ち約190行

## 2026-08-19 — CGC 混入を是正 + 収集用 Chrome を全経路で画面から消す

### ① CGC (鑑定会社) が素通りしていた (user 指摘)

- 原因は 2つ。 ①除外語を **`CCG` と書いていた** (正しくは `CGC`)。 しかも `CCG` は
  ガンダムの正規タイトル `Gundam CCG Edition Beta` (Collectible Card Game) に当たり、
  **まともなガンダム出品を弾く**副作用まであった。
  ②`\bCGC\b` は **`CGC10` にマッチしない** (数字が続くと単語境界が立たない)。
- 変更: 除外語を `BGS|ARS|CGC|SGC|AGS|BECKETT` に是正し、 境界を「前後に英字が無い」に変更。
  `contradicts_psa10()` を追加し、 **cert が読めている行は「PSA10 と書いていない」だけでは
  落とさない** (Vision がスラブを読めた正当な行を守る)。他社鑑定・10未満の明記のみ削除。
- 是正: 実データ 20行を削除 (onepiece 16 / dragonball 4)。他社鑑定の残り 0 を確認。
- ガンダム再走行で取りこぼしを回収: **49行 → 75行** (+26)。他ゲームも +18行。

### ② 収集用 Chrome が画面に出る (user 指摘・2回目で全経路対応)

- 8/18 の対応は「画面外へ飛ばす」だったが **効いていなかった**
  (実測: `-32000` 指定でも実際の位置は L=-7。 Windows が可視領域へ引き戻す)。
- 変更: **ウィンドウごと隠す** (`ShowWindow SW_HIDE`)。 描画は
  `--disable-features=CalculateNativeWinOcclusion` で続くので件数は落ちない
  (隠した状態で 30件/語 を確認)。最小化は描画が止まるので使わない。
- ★**1箇所直して終わりにしたのが誤り**。 `hide_browser_window` を `_chrome_util` に 1本化し、
  driver を起こす **全6箇所** (mercari / mercari_shops / amazon×2 / casio / snkrdunk) で呼ぶ。
  `uc.Chrome` を持つファイルに hide が無ければ落ちるテストを追加 = **呼び忘れをテストが捕まえる**。
- `IMAK_CHROME_ONSCREEN=1` で従来どおり画面に出せる (デバッグ用)。
...
緑 = HIGH にある行 / 白 = 未転記。N (仕入れ価格) と R (カテゴリ=TCG) は全行充填済で、
**行ごとコピーで HIGH に載せられる**。P (CTR) は HIGH 側の数式なので書いていない。

### 残務

- 再取得でも失敗した 111件 (ポケモン77 / ワンピース34)。URL は JSON に記録済。
- 目視待ち (I列空欄) 約190行。番号を入れれば出品対象になる。
- テスト 919 passed (offline) / 1,034 passed (pre-commit)。

## 2026-08-18 (続4) — 4ゲーム走行完了 + 品質の穴を3つ塞いだ

### 走行結果 (中間スプシ 合計 902行)

| タブ | 行数 | cert入り | 目視待ち(I列空) |
|---|---|---|---|
| pokemon | 431 | 365 | 66 |
| onepiece | 289 | 243 | 46 |
| dragonball | 125 | 100 | 25 |
| gundam | 49 | 35 | 14 |
| other | 8 | 8 | 0 |

- 拾い直し: ポケモン +4行 / ワンピース +57行。**再取得でも失敗した 111件** (ポケモン77 + ワンピース34)
  は削除済/閲覧不可と見られる。以後は `failed_urls` に URL が残る。
- ドラゴンボール+ガンダム: 候補151 + 目視待ち43。

### 塞いだ穴

1. **PSA10 以外の混入** (user 指摘): cert が読めない行をグレード未確認で入れていた。
   `psa_grade_gate` を新設 (他社鑑定 BGS/ARS/CCG/SGC、10未満、PSA10 表記なし は通さない)。
   既存 32行を削除。実走で `not_psa10_listing` 25件 + 12件 を新たに弾いた。
2. **途中書込が無かった** (user 指摘「途中で保存してよ」「前回も同じミス」): `--sheet-every`
   (既定30) で走行中にスプシへ書く。**書込失敗で走行を殺さない**。
   ★最初の実装が全タブ読み直しで **Google 読取上限 429 で走行が落ちた** → 既知キーは1回だけ読む。
3. **Chrome が画面に出て邪魔** (user 指摘): 非headless は必須なので **画面外に飛ばす**
   (`-32000,-32000`)。最小化は描画が止まるので不可。`IMAK_CHROME_ONSCREEN=1` で戻せる。

### 行ごとコピー対応 (HIGH)

- HIGH の PSA 行 1,310件を実測 → **N (仕入れ価格) と R (カテゴリ=TCG) が全件充填**、
  M (現在価格) は 447件空 = 監視が後で入れる列。**P (CTR) は `=countif(A:A,A行)` の数式**。
- 収集側で N/R を書くようにし、既存902行も backfill (`tools/backfill_high_ready_columns.py`)。
  **P は書かない** (値を貼ると重複検出が壊れる)。
- 既にコピー済みのワンピース226行は HIGH 側で N/P/R とも埋まっていることを確認済 (問題なし)。
...
- 決定 (user): 中間スプシは **ゲーム毎に4タブ** (`mercari_psa10_onepiece` 等)、
  重複チェックは **全 mercari_* タブ横断 + 本番**。
- 変更: `scrapers/psa_game.py` 新設 (Vision ラベル / 出品タイトル / 弾コード / カタログ和名 で判定、
  判らない物は `_other` へ = 捨てない) / `sheet_writer_mercari_search.load_keys_all_tabs()` /
  runner はゲーム毎に append / `tools/split_psa10_by_game.py` (既存行の移送、元タブは残す)。
- 実データで潰した判定バグ3件 (230行で実測):
  ① 先頭の `PSA10` を弾コードと誤認して打ち切り → 候補を全部見る
  ② `[OP08-106]` の括弧付きを取り逃がす → 分割ではなくパターンで抜く
  ③ 「ハンコック」の中の「コック」(別ゲームのカード名) に負ける → **最長一致**を採る
  結果 other 34 → **9件** (2文字名のサボ/カヤ等と遊戯王1件)。
- 移送実績: onepiece 218 / other 9 / gundam 2 / pokemon 1。元タブ `mercari_psa10` (230行) は
  **確認用にそのまま残してある**。中身を見てから消すこと。
- LOW を実測: PSA の cert 行は **0件** (HIGH に1,310件)。PSA の本番は HIGH のみ。
  URL 重複だけ LOW も見る (512行と軽い)。
- 検証: offline test 870 passed (+12)。
...
### 続き — 番号が取れなかった52件を回収 + 4ゲーム対応 (同日)

- 追加 (番号表記のゆれ): ポケモンの分数表記 `#077/071` `#232/S-P` と プロモ `P-053` を拾う
  → 52件中 **29件** を回収。
- 追加 (カタログ引き当て): `products` の `name_en → name_jp` で 残り23件中22件を和名に変換
  (`Nico Robin → ニコ・ロビン` 等)。**訳さない・推測しない、DB の lookup のみ**。
  ゲームが判らないタイトル (Weiss Schwarz 等) は変換しない。
- ★不具合を発見して修正: 部分一致だと `SIGNATURE` の中の `NATU` を拾って
  **別カード (ネイティ) に化けていた** → 単語境界照合 + 最長一致に是正。回帰テスト追加。
- 結果: 需要ベースのキーワードが **29語 → 74語** (既存の弾コード語との重複 0)。
- 4ゲーム対応: 需要側は `detect_game` (one_piece / pokemon / dragonball / gundam) で全対応。
  総当たり側も既に 88語 (onepiece 49 / pokemon 24 / dragonball 12 / gundam 5)。
- 検証: offline test 854 passed (+9)。実機でカタログ 6,325 名の 英名→和名 表を読込確認。
### 続き — キャラ軸も着手 (user 指示)

- 変更: 需要マップ (HQ 既存メンテ) の「属性別詳細」からキャラ行を読む
  (`parse_demand_map` / `build_character_keywords`)。判定が 🔵国内実需 / 🟢売れ筋 の行だけ採り、
  **🔴死筋 と 判定なしは採らない**。英名→和名は カタログ lookup のみ、和名が割れる英名は捨てる。
  国内OOS数 (在庫切れ本数) の多い順。
- 実測: キャラ行 169 → 採用対象 114 → **和名にできた 58語**
  (落ちた56件は `Son Gohan Childhood` `Shanks Wanted` 等の派生名でカタログに完全一致が無い)。
- 実機で表記の当たりを確認: `PSA10 モンキー・Ｄ・ルフィ` (カタログの全角Ｄのまま) で 20件。
  半角Ｄ版は**同じ結果**が返る (メルカリ側で正規化される) ので表記ゆれ対策は不要。
  通称 (`PSA10 ルフィ`) は 19件中 新規5件 = 大半が重複。**推測で通称を作る価値は薄い**ので入れない。
- 結果: 需要ベースのキーワードは **125語** (カード74 + キャラ58、うち重複7)。弾コード88語との重複0。
- 検証: offline test 858 passed (+4)。
