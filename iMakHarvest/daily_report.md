...
**夜(18:00〜)追記 — 門を3つ入れた / 方針が決まるまで全保留**

HQ 依頼 `treasure_gate_and_aj` / `treasure_new_targets` / `treasure_pause`。回答は各 `_response.md`。
- **AJ列の意味**: F列の仕入値 と `demand_market.csv` の「上限仕入れ値」を番号で引いた結果。
  ¥70,000 とは無関係。空=番号が読めない/一覧に無い (判定していないだけ)。HQ が「今のままで正しい」と確認
- **門3つ** (commit `a466c721` / `89f10780`、テスト1,308件通過。トレジャーハント専用):
  1. 仕入上限: HQ が共有領域に置く写し `C:/dev/iMak_data/shared/cost_sanity.json` を読むだけ (max_jpy=70,000)。
     **表が無ければ走る前に停止** (fail-closed)。Harvest の `iMakeBayAPI` は 4/25 の古い版で
     `cost_sanity` が無く、他の作業場所も読めないため、案B (写し) を HQ に頼んだ
  2. 外国語版 (中国語/英語/韓国語版) をタイトルで落とす
  3. カードごとの上限: `仕入値 <= 上限+7,000 かつ <= 上限x1.5` (ユーザー確定)。番号が読めない/一覧に無い
     は ¥70,000 の門だけ。HQ の表4例をそのままテスト化
- 門はどれも Vision の前に落とすので費用も省ける。捨てた理由は `foreign_edition` / `cost_over_limit` / `card_cost_over`
- **既にシートに入った 1,129件は消さない** (HQ 回答: 出品手前の監査が落とす。集めたデータは資産)
- `demand_market.csv` は走行のたびに読み直す (焼いていない)

**失敗**: 門の確認に `--dry-run` を撃ったが、dry-run も実検索する (シートに書かないだけ) ため走行が始まった。
数秒で止め Chrome も片付けた。シートへの書込みは無し。dry-run は「実検索する」と skill に既に書いてあったのに
確認せず撃った

**保留 (ユーザー判断 2026-09-20)**: HQ が「売れ筋を探す」戦略を見直し中 (実測: 221件中そのまま利益が出るのは
17件=8%。狙いを「今誰も出していない」へ変えるか検討し、ACTIVE データを取る)。
**方針が決まるまで全て保留**: 夜間/週次の予約 (未登録のまま)、手での走行、取得失敗220件の拾い直し
(`--retry-from-json` もまだ足していない)。再開は HQ の再依頼待ち。
