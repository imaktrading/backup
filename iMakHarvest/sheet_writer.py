...
# 補仕入連携時の列数 (= AC-AG 含む 33 列まで拡張)
WITH_AUX_COLUMN_COUNT = 33
...
def _build_row(item: dict) -> list:
    ...
    - 触らない列 (B/D, I-R, U-AB) は "" にして既存値の上書きを避ける
      (※ append_rows は新規行のみ追加するので、新規行の触らない列は空欄になるだけ)
    - auxiliary_urls (= 補仕入連携で取得した PSA10 URL list) があれば AC-AG (29-33) に
      最大 5 件まで左詰めで書込
    """
    ...
    aux_urls = [str(u) for u in (item.get("auxiliary_urls") or []) if u]

    width = WITH_AUX_COLUMN_COUNT if aux_urls else DEFAULT_COLUMN_COUNT
    row = [""] * width
    ...
    row[COL_SIZE - 1] = size
    # U-AB (21-28) は空欄
    # AC-AG (29-33) に auxiliary_urls を最大 5 件左詰め
    for i, url in enumerate(aux_urls[:5]):
        row[COL_AUX_URL_1 - 1 + i] = url
    return row
...
def append_new_urls(
    ...
    # batch 内に auxiliary_urls 含む item があれば column_count を 33 列に拡張
    if any(it.get("auxiliary_urls") for it in items):
        column_count = max(column_count, WITH_AUX_COLUMN_COUNT)
