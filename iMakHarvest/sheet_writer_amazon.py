...
# ★HIGH の N列は **数式** (`=ARRAYFORMULA(...)` が N1 から下まで流れている)。
# 2026-08-18 に セル単位で見て「値だ」と誤読し、 中間スプシの N を埋めた。
# 中間スプシに書く分には害は無い (別シート) が、 **HIGH へコピーする時は N と P を貼らない**
# (2026-08-19 HQ 確認。 N に値を書くと spill が塞がり列全体 1,400行超が空になる)。
# 仕入れ値を本番に入れたい時は M列 (現在価格・観測値) に書く。
COL_PURCHASE_PRICE = 14  # N: 仕入れ価格(円)      - 中間スプシのみ。 HIGH では数式
