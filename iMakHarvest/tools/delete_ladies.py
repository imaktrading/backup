"""中間スプシ amazon_gshock から レディース専用 行を物理削除 (= メンズ scope 維持).

2026-06-12 user 指示: メンズ preset に混入したレディース 50件を削除しメンズのみに戻す。
判定は scrapers.amazon_search_http.is_ladies_only (= keep gate と同一基準、 単一ソース)。
兼用 (= メンズ併記) は削除しない。

実行:
  python tools/delete_ladies.py --dry-run
  python tools/delete_ladies.py
"""
...
from scrapers.amazon_search_http import is_ladies_only  # noqa: E402
...
def main() -> int:
    ...
    print(f"[delete-ladies] deleted={deleted}", flush=True)

    (ROOT / "debug" / "delete_ladies_result.json").write_text(
