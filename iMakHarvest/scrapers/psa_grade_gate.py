...
2026-08-18 新設 (user 指摘「PSA9 や BGS9.5、ARS、CCG が混じっているね」)。

事故の形: cert が読めなかった出品を **グレードを見ずに** I列空欄で入れていたため、
PSA9・BGS・ARS・CCG・生カード (鑑定なし) まで中間スプシに並んでいた。
メルカリ検索は説明文にも当たるので、 タイトルに PSA10 が無い出品も普通に返ってくる。

方針は 出品の正確性原則どおり **fail-closed**:
  - 別の鑑定会社 (BGS / ARS / CCG / SGC / AGS) が書かれていたら通さない
...
OTHER_GRADER_RE = re.compile(
    r"\bBGS\b|ベッケット|\bBECKETT\b|\bARS\b|\bCCG\b|\bSGC\b|\bAGS\b|\bTAG\b",
    re.IGNORECASE)
