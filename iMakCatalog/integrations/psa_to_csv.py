...
_POKEMON_SIBLING_SETS: dict[str, tuple[str, ...]] = {
    ...
    "DPs-S": ("DPs-S-G",),
    "DPs-S-G": ("DPs-S",),
    # スタートデッキ100 コロちゃおVer. は M-P と同じ画像フォルダだが別の通し番号。
    "M-P": ("M-P-KC",),
    "M-P-KC": ("M-P",),
    # スタートデッキ100 (SI) とスタートデッキ100 コロコロコミックver. (SN) は番号が衝突
    # (SI-008=キノココ / SN-008=カビゴン)。Brand の「COROCORO COMIC VERSION」だけでは
    # SI として先にヒットするので、名前不一致時に SN 側を試す (2026-09-10)。
    "SI": ("SN",),
    "SN": ("SI",),
}
