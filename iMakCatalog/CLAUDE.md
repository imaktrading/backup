...
### 扱わない商材 — 起票が来ても却下してよい (2026-08-31 ユーザー確定)

| 商材 | 決定 | 経緯 |
|---|---|---|
| **ヴァイスシュヴァルツ** | **今は扱わない** | 2026-08-31 ユーザー判断。PSA再仕入れの不一致に2件出た (NIKKE の RRR / SP Signature) が、新カテゴリの立ち上げ (公式カードリストの所在調査から) が必要で、今その投資はしない |

catalog が持つのは **9カテゴリだけ**:
`yugioh_tcg` / `pokemon_tcg` / `one_piece_tcg` / `dragonball_scg` / `gundam_tcg` /
`montbell` / `gshock` / `uniqlo_ut` / `workman`。

- 上に無い商材が「カタログ未登録」で上がってきたら、**欠落ではなく対象外**。
  行を足さず「対象外シリーズ」と返す
- 扱いたくなったら、**ユーザーの判断が先**。公式カードリストの所在から調べる話になる
...
### セット名の誤りを見つける仕組み — **見た目の条件で確認しない** (2026-08-23 制定)

**事故**: 英語版セット名が 1,729行に入っていたのに、こちらは「`Swsh` で始まる値」という
**思いついた条件**で確認して「0行」と報告した。`Sun & Moon—Celestial Storm` /
`Scarlet & Violet—Mega Brave` / `The Best of XY`(別セット) が漏れていた。
HQ が疑わなければ、そのまま出品されていた。

**教訓**: *条件を思いつけるかに依存する確認は必ず漏れる。* 許可された値の一覧と突き合わせる。

#### 仕組み (3つ。どれも毎日の監査に常設・0件で維持)

| 面 | 何を見るか | なぜ必要か |
|---|---|---|
| **§0** 弾コード食い違い | eBay 値の頭の弾番号 ≠ 商品の弾コード | 変換表が誤っていると §6 は素通り (derive も同じ誤りになる) |
| **§0b** 未登録のセット名 | ① eBay master にも ② 登録簿にも無い値 | **新しい誤りは登録されていないので必ずここに出る** |
| §6 canonical ズレ | 変換表から計算した値 ≠ 焼いてある値 | 変換表を直したのに焼き直し忘れ |

`set_name_ebay` に入ってよい値は2つだけ:
```
① eBay master (Game 別) に在る値
② ebay_filter_map/_free_text_set_values.yaml に登録した値 (162値)
```

- 登録簿は `tools/build_free_text_registry.py` で作り直す
- **英語版シリーズ名で始まる値は登録できない** (生成時に弾く)
- 回帰 `tests/test_set_value_registry_20260823.py` / `test_setname_prefix_mismatch_20260823.py`

### eBay "Set" 欄に何を入れるか — **確定 (2026-08-23 ユーザー確定「シンプルが一番」)**

```
① eBay にそのセットの値がある   → それをそのまま入れる (master の verbatim)
② 無い                          → 日本語セット名の英語表記を自由入力で入れる (空欄にしない)
③ 英語版の別セット名を使う      → 禁止。**例外は作らない**
```

**これで終わり。分岐はこの3行だけ。**

#### 2026-08-18 の例外 (英語版セット名 14セット) は廃止した

「日本語版2セットが英語版1セットに併合される分は英語版名を使う」という裁定があったが、
**実測したら14セットとも eBay に日本語版セットの値が在った**ので例外は不要だった。

    漆黒のガイスト  Swsh06: … Chilling Reign  ->  S6k: Jet-Black Spirit
    白銀のランス    Swsh06: … Chilling Reign  ->  S6h: Silver Lance
    ロストアビス    Swsh11: Lost Origin       ->  S11: Lost Abyss     ほか11セット (計 1,361行)

#### 新弾のときにやること (これだけ)

1. 取り込みスクリプトを走らせる
2. **変換表に1行足す** — eBay に値が在れば①、無ければ②
3. 新しいレアリティ記号が出たら rarity に1行足す

★新弾の直後は eBay 側に値が無いのが普通なので、**ほぼ②になる**。空欄にしないこと。

#### 検知
`tools/set_name_integrity_audit.py` の **§0 弾コード食い違い**が、別セットの名前が
入っていないかを毎日見る。0件で維持する。§6 (canonical ズレ) では**捕まらない**
(変換表そのものが誤っていると計算し直した値も同じ誤りになるため)。

### 旧: ルール案 (2026-08-21 / 廃止・履歴として残す)
...
### 項目の仕分け — 見張る / 閉じる (2026-08-25)

全体表: `iMak_data/catalog/requests/2026-08-25_aspect_triage_21_items.md`

#### 見張っている項目 (毎日の監査で 0 を維持)

| 項目 | 監査の節 |
|---|---|
| Set | §0 弾コード食い違い / §0b 未登録の値 / §0c 別セットの名前 |
| Rarity | §7 生値焼き付き・map ズレ / §8 レアリティでない値 |
| Stage | §9 進化段階を持たない種別に stage |
| Card Type | §10 eBay の値表に無い値 |
| Game / Manufacturer / Card Size | §11 定数が category ごとに1値か |
| Card Name / Character | HQ `psa_name_watch.py` (PSA ラベル突合。外の正解が要るのでこちらでは作れない) |

#### 閉じた項目 — 監査に出ても穴ではない。起票が来たら却下してよい

| 項目 | 決定 | 根拠 |
|---|---|---|
| **Creature/Monster Type** | **出さない** | 遊戯王専用の項目で、当社は遊戯王を1枚も出していない。全カテゴリ 0% (2026-08-25 実測)。出品CSVに列も無い |
| **Illustrator** | **取れるカテゴリだけ**出す | ポケモン 21,140/22,111。ワンピは元データに項目が無く 0/8,726。無い所を埋める手段が無い |
| **Speciality** | **ポケモンだけ**出す | EX / V / GX / VMAX / BREAK をカード名から取る。他ゲームに相当する概念が無い |

★`Features` は保留 (2026-08-25)。出品側が複数値を読点で1セルにしていた不具合を修正中で、
  実物 (GetItem) で確認できてから閉じる。回答: `requests/2026-08-25_hq_answer_features_multi_value.md`

★既に「出さない」と決めてある項目 (再掲): Age Level / Autographed / Finish

#### eBay の値リストの読み方 (2026-08-25 — 一度間違えたので明記)

`_input/ebay_aspects_183454_latest.json` の値は **`aspects.<項目>.all`** に入っている。
`values` というキーは無い。`by_game` は Game 別の内訳で、持っているのは Set と Card Type だけ。

★**一覧が在っても「許可リスト」ではない。** `constraint.mode` を見ること。
  カタログが出す21項目のうち **20項目が `FREE_TEXT`** で、一覧は候補にすぎず
  外れても eBay は弾かない (絞り込みに乗らないだけ)。
  選択専用 (`SELECTION_ONLY`) は **Country of Origin だけ** (`Japan` は在るので問題なし)。

  → だから「一覧に無い = 誤り」ではない。誤りなのは **その弾/カードに対応する値が
    一覧に在るのに別の値を使っている**時 (= Set の §0c と同じ形)。

★Stage の一覧9値のうち Champion / Rookie / Ultimate / In-Training / Hybrid は
  **デジモン**の進化段階。ポケモン向けは Basic / Stage 1 / Stage 2 / Mega の4つだけで、
  `V進化` / `レベルアップ` / `BREAK進化` / `復元` / `伝説` / `V-UNION` に**対応する値は無い**。
  空欄が正しいのは「推測になるから」ではなく「**当てる先が無いから**」。

#### ★出力の汚れを止められるのはカタログだけ (2026-08-25 HQ 実測)

HQ の CSV 監査は **カタログ自身の値集合を基準にしている**ため、汚れた値ごと通る。
実測: `check_csv._catalog_set_consistency('Movie Promo', ...)` → `None` (素通り)。
= 「出品側でも止まる」を前提にしない。**検知面はこちらに作る。**

### 公式カードリストに無いカードを登録してよいか (2026-08-24 ユーザー確定)

**登録してよい。ただし値の出所を限る。**

現物が存在して PSA が鑑定していれば、バンダイの公式カードリストに未掲載でも登録する
(未掲載のまま放置すると、出せるカードが毎回「該当なし」で落ちる)。

| 値の出所 | 可否 |
|---|---|
| **発売元の公式ページ** (集英社の書籍ページ 等) | **可** |
| **PSA スラブ実写で読める券面** (番号 / レアリティ / 名前 / コスト等) | **可** |
| 同じカード番号の公式行からの複製 (効果文・特徴の英訳 等) | 可。ただし **複製と分かる印**を残す |
| ショップ / wiki / まとめサイト | **不可** |

★`source` に出所が分かる文字列を必ず入れる (例 `shueisha_chopper1_bundle+psa_cert168157614`)。
★**公式が載せたら公式値で上書きする**。migration の docstring にその旨を書く。

先例: `EB02-003_CH01` (集英社『ONE PIECE CHOPPER’s 1』付録) /
`ST13-003_7E01` (セブンイレブン配布) / `OP06-068_AC01` (Admirable Collection vol.1)

#### 画像だけは別扱い

公式に絵が無い時は **第三者画像を目視照合用に入れてよい** (下の「第三者 source 例外規約」の4条件)。
ただし**対症療法**であることを注記に書き、公式が絵を出したら差し替える。

★根治は **目視画面に `set_name_official` を出すこと**。PSA ラベルと文字で一致するので
  絵が無くても選べる (`ONE PIECE CHOPPER'S 1` ↔ 『ONE PIECE CHOPPER’s 1』付録)。
  画面は HQ の持ち物なので依頼済 (`iMak_data/hq/requests/2026-08-24_review_screen_show_setname.md`)。
