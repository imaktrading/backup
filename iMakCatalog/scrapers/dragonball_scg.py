#!/usr/bin/env python3
"""Dragon Ball Super Card Game (Fusion World) scraper — Bandai TCG+ API 経由.

データ源:
  - https://api.bandai-tcg-plus.com (game_title_id=10 EN / 11 JA)

設計:
  - list endpoint をページングで全件取得 (EN/JA 別)
  - 各カード詳細は disk cache (db/cache/) に保存して再取得を回避
  - バリアント (通常/Leader_F/再録) は別レコードとして保存し、product_id に suffix
  - JA は image_url から正規化キーで EN にマージ
  - image filename: 'EN_FW_FB09-001_Leader_F_dummy.png' / 'JP_FW_FB09-001_Leader_F_dummy_s.png'

CLI:
  python scrapers/dragonball_scg.py --full              # 全件 (約 5700 件、~2.5 時間)
  python scrapers/dragonball_scg.py --update            # 差分のみ
  python scrapers/dragonball_scg.py --card FB09-001     # 単一 card_number
  python scrapers/dragonball_scg.py --limit 20          # 先頭 N 件のみ
"""
...
def _normalize_image_path(image_url: str) -> tuple[str, str]:
    ...
    例 (Dragon Ball Fusion World):
      https://.../DBFW-EN/FB09/EN_FW_FB09-001_Leader_F_dummy.png → ("FB09", "FB09-001")
      https://.../DBFW-EN/FB09/EN_FW_FB09-001_Leader_F_PARA_dummy.png → ("FB09", "FB09-001_PARA")
      https://.../DBFW-JA/FB09/JP_FW_FB09-001_Leader_F_SUPERPARA_dummy_s.png → ("FB09", "FB09-001_SUPERPARA")
      https://.../DBFW-EN/FB09/EN_FW_FB09-002_Battle_UC_dummy.png → ("FB09", "FB09-002")

    DBSCG image filename には card_type+rarity を表す中間タグ (`_Leader_F` /
    `_Battle_C` / `_Battle_UC` / `_Extra_R` 等) が固定で入る. これらは
    card_config の Type/Rarity と重複情報なので variant suffix から除去し、
    本物のバリアントマーカー (`PARA` / `SUPERPARA`) のみ残す.
    """
    if not image_url:
        return "", ""
    ...
    # 言語+ゲームタグ prefix
    fname = re.sub(r"^(EN|JP)_FW_", "", fname)
    fname = re.sub(r"^batch_", "", fname)
    # placeholder/dummy suffix
    fname = re.sub(r"_dummy(_s)?$", "", fname)
    fname = re.sub(r"_d$", "", fname)
    # DBSCG 固有: card_type + rarity の中間タグを除去 (重複情報、card_config に同等値あり)
    #   _Leader_F / _Leader_C / _Battle_C / _Battle_UC / _Battle_R / _Battle_SR / _Battle_SCR
    #   _Extra_C / _Extra_R 等
    fname = re.sub(
        r"_(Leader|Battle|Extra|Unison)_[A-Z]+",
        "",
        fname,
    )
...
# JA 版 detail の config_name キーを EN 表記に正規化 (Dragon Ball Fusion World 固有スキーマ).
JA_CONFIG_KEY_TO_EN: dict[str, str] = {
    "カード種類":    "Type",
    "レアリティ":    "Rarity",
    "色":            "Color",
    "合計コスト":    "Energy",
    "指定コスト":    "Specified Cost",
    "パワー":        "Power",
    "コンボパワー":  "Combo power",
    "特徴":          "Special Trait",
    "備考":          "Notes",
...
# ============================================================================
# 公開準備中 / 言語タグ marker (= OPCG / Gundam と同 logic 共有)
# DragonBall URL 例:
#   DBFW-EN/FB09/EN_FW_FB09-121_Battle_SCR_SUPERPARA_dummy-2.png
#   DBFW-JA/FB09/JP_FW_FB09-121_Battle_SCR_SUPERPARA_dummy_s.png
#   DBFW-EN/FB06/EN_FW_FB06-071_Leader_F_dummy_nonotforsale.png
# EN/JA で `_dummy_s` / `_dummy_nonotforsale` 等 pre-release marker が
...
def _strip_ja_prelim(leftover: str) -> str:
    """pre-release / 言語 marker (= _sample / _dummy / _s / _nonotforsale 等) を除外.

    parallel marker (= `_PARA` / `_SUPERPARA`) や variant identifier
    (= `_SCR` / `_F` 等 rarity) は除外対象外で variant 識別性維持。
    """
    prev = None
    cur = leftover
    while cur != prev:
...
def build_and_upsert(
    ...
    # OP/Gundam/DBFW は printed number = code (例 SB02-033)。2026-06-21 HQ依頼で scraper 補完。
