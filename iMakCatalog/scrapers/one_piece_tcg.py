#!/usr/bin/env python3
"""One Piece Card Game scraper — Bandai TCG+ API 経由で全カードを iMakCatalog DB に投入.

データ源:
  - https://api.bandai-tcg-plus.com (game_title_id=4 EN / 8 JA)

設計:
  - list endpoint をページングで全件取得 (EN/JA 別)
  - 各カード詳細は disk cache (db/cache/) に保存して再取得を回避
  - バリアント (通常/Parallel/再録/特殊仕上) は別レコードとして保存し、product_id に suffix
      base                : OP06-022
      parallel            : OP06-022_p
      reprint in ST28     : OP06-022_ST28
      reprint EB02 + LF   : OP06-022_EB02_LF
  - JA は image_url から正規化キーで EN にマージ (JA 限定プロモは name_jp のみのレコード)

CLI:
  python scrapers/one_piece_tcg.py --full              # 全件 (約 4500 件、~50 分)
  python scrapers/one_piece_tcg.py --update            # 差分のみ (DB に無い product_id だけ)
  python scrapers/one_piece_tcg.py --card OP06-022     # 単一 card_number の全バリアント
  python scrapers/one_piece_tcg.py --limit 20          # 先頭 N 件のみ (動作確認用)
"""
...
def _normalize_image_path(image_url: str) -> tuple[str, str]:
    ...
    例:
      https://.../card_image/OP-EN/OP06/OP06-022_d.png         → ("OP06", "OP06-022")
      https://.../card_image/OP-EN/OP06/OP06-022p_d.png        → ("OP06", "OP06-022p")
      https://.../card_image/OP-EN/ST28/batch_OP06-022.png     → ("ST28", "OP06-022")
      https://.../card_image/OP-EN/EB02/batch_OP06-022_LF_d.png→ ("EB02", "OP06-022_LF")
      https://.../card_image/OP-JA/P/batch_OP06-022.png        → ("P",    "OP06-022")
    """
    if not image_url:
        return "", ""
...
# JA 版 detail の config_name キーを EN 表記に正規化 (固定スキーマ翻訳のみ、
# 値は raw 保存。CHARACTER 名などの辞書化禁止対象とは別物).
...
def build_and_upsert(
    ...
    # OP/Gundam/DBFW は printed number = code (例 OP09-037)。2026-06-21 HQ依頼で scraper 補完。
