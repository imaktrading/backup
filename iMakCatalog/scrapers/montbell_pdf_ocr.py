...
def _load_anthropic_key() -> str:
    """API key.txt を 既存リポジトリ内の数箇所から読み込み.

    worktree 配下では gitignore で API key.txt が同期されないため、
    本元 worktree (C:/dev/iMak/) も fallback で参照する.
    """
    candidates = [
...
各 page には 0〜複数の商品が掲載されている. 各商品について以下のフィールドを抽出する:
  ...
  - outer_shell_jp:     表地 (素材表記の "表地:" 直後、または素材が単一表記なら本文ごと). 例: "ナイロン・タフタ"
                        ※ 表地サブタグなしで「素材：40デニール・ナイロン・タフタ」と単独表記の
                          場合も、その素材文字列を outer_shell_jp として返すこと.
  - lining_jp:          裏地 (素材表記の "裏地:" 直後). null 可.
  - insulation_jp:      中わた (素材表記の "中わた:" 直後). null 可 (アウター系では多くが null).
  - country_jp:         原産国 (例: "日本", "中国", "ベトナム", "ミャンマー", "バングラデシュ" 等).
                        page 上に "原産国" や "Made in 〇〇" 表記があれば必ず取得. null 可.
...
def _normalize_ocr_product(p: dict, source_label: str) -> Optional[dict]:
    ...
    country = mb._translate_first_match(p.get("country_jp") or "", mb._COUNTRY_JP_EN, "Not Specified")
...
def _upsert_normalized(np: dict, source_label: str):
    ...
    既存 record が存在する場合 (例: 先に disp_fo で投入済) は _merge_specs で
    空欄のみ補完 (実値は保持). source / source_url は既存値を維持し、
    pdf_ocr_augmented フラグを立てる.
    """
    ...
    existing = api.lookup(CATEGORY, np["product_id"])
    if existing:
        merged = mb._merge_specs(existing.get("specs", {}), new_specs_block)
        merged["pdf_ocr_augmented"] = True
        api.upsert(
            category=CATEGORY,
            product_id=np["product_id"],
            name=existing.get("name") or np.get("name_jp", ""),
            name_jp=existing.get("name_jp") or np.get("name_jp", ""),
            specs=merged,
            images=merged.get("image_urls", []),
            source=existing.get("source"),  # 既存 source 維持
