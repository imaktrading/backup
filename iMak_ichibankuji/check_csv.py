#!/usr/bin/env python3
"""
iMak Trading Japan - eBay CSV チェッカー (一番くじ)
...
RECOMMENDED_SPECIFICS = [
    "C:Material", "C:Theme", "C:Color", "C:Size",
]

# ===== 利益計算パラメータ（利益計算シート_v2.xlsx準拠） =====
PROFIT_PARAMS = {
    "exchange_rate": 159.245,   # USD→JPY（設定シート B2）
    "ebay_fee_rate": 0.185,     # US実効手数料率（設定シート B9）
    "promo_rate": 0.1,          # 実効プロモ率（設定シート B3）
    "payo_rate": 0.025,         # ペイオニア手数料（設定シート B4）
    "shipping_jpy": 2500,       # 一番くじ実送料（設定シート C17）
}
# net_ratio = 1 - 0.185 - 0.1 - 0.025 = 0.69
...
def search_ebay_active(token, keywords, limit=50):
    """Browse API で同一商品のアクティブ出品を検索（最大50件）。
    ...
    params = {
        ...
        "filter": (
            ...
            "categoryIds:{261055}"  # Collectible Figures
        ),
        "sort": "price",
        "limit": min(limit, 200),
    }
...
EBAY_SPEC_TO_CSV = {
    "Brand": "C:Brand", "Character": "C:Character", "Type": "C:Type",
    "Franchise": "C:Franchise", "Material": "C:Material",
    "Theme": "C:Theme", "Color": "C:Color", "Size": "C:Size",
}
...
def build_search_query(row):
    ...
    franchise = get_col(row, "C:Franchise")
    item_type = get_col(row, "C:Type")

    query = f"Ichiban Kuji {franchise} {character} {item_type}"
    return query.strip()
...
def validate_row(row, row_idx):
    ...
    # --- カテゴリ・条件 ---
    if category != "261055":
        issues.append(("ERROR", f"カテゴリが 261055 でない: {category}"))
    if condition != "1000":
...
def claude_review(rows, all_issues, all_comp_findings, all_gates):
    ...
    for i, row in enumerate(rows):
        title = get_col(row, "*Title")
        price = get_col(row, "*StartPrice")
        brand = get_col(row, "C:Brand")
        character = get_col(row, "C:Character")
        franchise = get_col(row, "C:Franchise")
        item_type = get_col(row, "C:Type")
        material = get_col(row, "C:Material")
        theme = get_col(row, "C:Theme")
        ...
        summary_lines.append(
            f"#{i+1} Title: {title}\n"
            f"   Price: ${price} | Brand: {brand} | Character: {character} | Franchise: {franchise}\n"
            f"   Type: {item_type} | Material: {material} | Theme: {theme}\n"
            f"   {issues_text}{comp_text}{gate_text}"
        )

    prompt = f"""You are an eBay listing quality reviewer for Japanese Ichiban Kuji prize figures and collectibles.
...
# ===== メイン =====
def main():
    print("=== iMak Trading Japan - eBay CSV チェッカー (一番くじ) ===\n")
    ...
    if cost_data:
        ...
        print("仕入値: コストデータなし（コストJSONファイルがあるとGATE判定が有効になります）")
