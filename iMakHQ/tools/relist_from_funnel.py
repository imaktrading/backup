...
def relist_candidates(rows):
    """funnel CSV rows から RELIST フラグ行 (NO_SEARCH ∩ watcher無) を抽出。"""
...
def main():
    ...
    print(f"取下再出品(relist)候補 = NO_SEARCH ∩ watcher無 = {len(cands)}件")
    if not cands:
        print("候補なし。処理終了。")
        return
    ...
    with open(cand_path, "w", newline="", encoding="utf-8-sig") as f:
        ...
        w.writeheader()
        for r in sorted(cands, key=lambda x: -float(x.get("price") or 0)):
            w.writerow(r)

    print(f"\nEnd CSV (eBayアップで取下げ): {end_path}")
    print(f"候補一覧(確認用): {cand_path}")
    print("\n▶ 取下げ: End CSV を eBay FileExchange にアップ → 該当listingが終了(Cassini reset)")
    print("▶ 再出品: 各カテゴリの listing スクリプトで再Add (既存 drop-ship flow)")
    print("※ watcher有は候補外 (relistするとwatcher消失→タイトル編集等 in-place 推奨)")
