...
def build_prompt(stamp: str) -> str:
    return (
        ...
        f"0. **下書きの仕分け: `python {TOOLS / 'draft_triage.py'} --record`** ← 最初にこれ。\n"
        "   `--record` で推移が `triage_metrics.jsonl` に残る。**出力の『🚩 体制の見直しライン』"
        "が出たら、レポートの『窓口が今すぐ見るべきもの』の先頭に必ず転記**する"
        "(現体制を続けるか案1へ進むかの判断材料。窓口が覚えていなくても届くようにするため)。\n"
