...
def _build_prompt(wt: str, pending: list[Path]) -> str:
    ...
    return (
        ...
        "【文量・厳守】書く文章は**短く**。draft は要点だけにする。\n"
        "- 結論を先に1〜3行。根拠は**実測値そのもの**だけを最小限。長い表・見出しの積み重ねは禁止。\n"
        "- 依頼書の『既に判明していること』を書き写さない。\n"
        "- 案を A/B/C と並べない。推す1案と、その理由1行。\n"
        "- ★長い文章は読む側 (人間も他セッションも) のトークンを食う。"
        "2026-08-11 ユーザー指示で全セッション徹底。\n\n"
        "- ★**ユーザーへの実況中継をしない**。依頼・回答は担当者に直接返して処理を進める。\n"
        "  ユーザーに上げるのは**判断が要る時だけ**で、その時は『何を決めてほしいか』を1行で書く。\n"
        "  『〜が来ました』だけの報告は禁止 (2026-08-11 ユーザー指示)。\n"
...
def _build_implement_prompt(wt: str, responses: list) -> str:
    ...
    return (
        ...
        "  投入は窓口が宛先を確認してから行う。\n\n"
        "【文量・厳守】書く文章は**短く**。依頼書/回答書/draft は要点だけにする。\n"
        "- 結論を先に1〜3行。根拠は**実測値そのもの**だけを最小限。長い表・見出しの積み重ねは禁止。\n"
        "- 相手が既に書いた実測を書き写さない (『既に判明していること』は再掲しない)。\n"
        "- 案を A/B/C と並べない。推す1案と、その理由1行。\n"
        "- ★長い文章は読む側 (人間も他セッションも) のトークンを食う。"
        "2026-08-11 ユーザー指示で全セッション徹底。\n\n"
        "- ★**ユーザーへの実況中継をしない**。依頼・回答は担当者に直接返して処理を進める。\n"
