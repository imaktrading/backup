...
try {
    $job = (Invoke-WebRequest -Uri 'http://127.0.0.1:8765/api/log?after=0' -UseBasicParsing -TimeoutSec 10).Content | ConvertFrom-Json
} catch { $job = $null }
...
try {
    $v = (Invoke-WebRequest -Uri 'http://127.0.0.1:8765/api/version' -UseBasicParsing -TimeoutSec 20).Content | ConvertFrom-Json
