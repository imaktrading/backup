...
### カタログの引き方 — **tools/catalog_lookup.py が唯一の口** (2026-09-20)

「カタログに無い」と言う前に、**引き方を尽くしたか**を必ず確かめる。
実測: 「カタログ要補充 17件」と出していたが、**16件はカタログに在った** (= ②だった)。

    python iMakHQ/tools/catalog_lookup.py "<市場のタイトル>"   # 1本で引き方を確かめられる

引く順番・番号を読む時の落とし穴 (PSA10 の 10 / 年 / eBay ライブの日付)・
画像の落とし穴 (英語版を先頭に持つ物 / PSA 鑑定画像しか無い物) は、
**あちらの docstring に全部書いてある**。他の道具は自前で引かない。
