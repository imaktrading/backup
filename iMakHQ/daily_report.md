...
## 2026-09-19 20:00 [HQ] 追記 — 「同じ出品が何度も目視に戻る」の真因はスニダンのURL

### 分かったこと

ユーザー報告 (820065007761 ミモザ / 820133533434 神龍) を追った結果、**2つ別の話**だった。

1. **並びが毎回同じ**だったので、245件あっても上位15件しか目に入っていなかった
   → 今日入れた「前に出した順に回す」で解消。この2件は今 214位 / 227位まで下がり、
     **まだ一度も出していない出品が151件**ある
2. **スニダンの個体違いを「新しい供給」と数えていた** (真因・commit c9018b38)
   スニダンは同じカードに個体ごとの URL を付ける
   (`/apparels/761394/used/50273173` … `/used/49135740`)。売れて新しい個体が並ぶたびに
   URL が変わるので、**毎回「新しい供給が出た」**と判定して判定済みの出品を出し続けていた。
   実測: 神龍の候補8本のうち **5本が同じ商品IDの別個体**。
   19:42 の走行ログにも「判定済みだが新しい供給が出た 11件」と出ていた
   → 供給の同一性は **商品ID (apparels/NNN) まで**で見る。メルカリは1出品=1個体なので従来どおり

### 今日の出品

- 入稿CSV **17行** (tcg_upload_20260919_192658.csv)。エラー0・除外0。
  18→17 は仕入元URLが既存出品と重複した1行が自動で外れたもの (昨日と同じカード)
- 補URL目視を5回走らせ、**51〜53本ずつ追記**。「違う」は 5件 → 1件に減った

### 数字

- **クリック率・購入率が 0.2% に上昇** (ユーザー確認)。棚卸し (売れない在庫を落として
  枠を空ける) が効いている。明日トレジャーハントが回れば「売れているのに出していない
  カード」が入るので、**落とす側と入れる側の両方**が回り出す

---

## 2026-09-19 19:30 [HQ] 補URL の目視を作り直した — 「同じカードばかり出る」の原因3つを全部塞いだ

### 決定 (ユーザー確定)

- **補が3本以下は 今の仕入値の1.5倍まで**出す (供給が1本も無いより、高くても買える先がある方がよい)。
  4〜5本の入れ替えは今までどおり **¥1,000以上安い**分だけ。**値段が分からない候補は出さない**
  (「入れ替えに高いのが出てきたら本末転倒」)。★¥70,000 の上限 (cost_sanity) はその下で生きている
- **目視は「前に出した順」で回す**。並びが毎回同じで、1回の件数を超えた分が毎回隠れていた
- **eBay の日付は日本時間17時以降なら通年で新しい日**。出品作業はそれに合わせる
- 今日やること の「全部出す」は**ユーザーの発言ではなかったので取り消した** (revert c443d307)

### 「同じカードばかり目視している」の真因 (3つ)

| # | 原因 | 直し方 |
|---|---|---|
| 1 | 監視くんが売り切れた補URLを空にする → 補が減って補充対象に戻るが、**そのURLは売り切れと記録されていない**ので候補にまた出る | 監視くん側で not_buyable に登録 (356件 → 2,626件) |
| 2 | 再入荷する仕入元 (Shops/Amazon) は登録しない判断 → 今も候補に出る | 落とさず**後ろに回して「今は売り切れ (再入荷あり)」と表示** (283件) |
| 3 | **「見送り」にした候補がどこにも記録されない** (記録していたのは「違う」だけ) | 理由つきで記録し、**値段が下がるまで出さない** |

3は自分たちの不具合。出品を確定した走行では、チェックを外しただけの候補が記録されず、
翌日また同じ候補が並んでいた。

### 変更 (今日の commit 26本 / 主なもの)

| commit | 内容 |
|---|---|
| e1ee518f / d3625d38 | 補URL の書込台帳を**配線**し、**減った分も残す**。2026-09-08 に作られていたが一度も呼ばれておらず、ファイルすら無かった |
| 7cf5f043 / ee13a352 | 見送りを記録。**8列目は前から別用途でも使われていた**ため「見送り以外は全部NG」に訂正 (175行のNGが効かなくなるところだった) |
| d61d38fb / de70d37b | 値段の門を決め直し + 前に出した順で回す |
| 9ef80fdc | 変種違いを**逆向き**でも外す (現物がSPで候補が通常版)。安全側 = 両方に表記があって食い違う時だけ |
| 24222f64 / a473dd67 | 目視待ちの候補に**値段と名前**を出す。空のまま出ていたため、中身を見ずに「違う」(永久NG) を押していた |
| 871088a3 / cfda2cfe | 候補と現物を**前へ/次へ**で送る。スクロールバーは非表示 |
| 76c80f5b / 524416dc / eeb8dcea | 監査くんの提案4本を実装 (スポーツカードの素通り / ハイフン名が消える / 空振り照合 / Act の結論が翌日消える) |
| 3dddcc14 / b0c53f27 / f954eaca | 9/14 から置いたままだった残務3本 (№41 / №202 / №63 の一部) |

### 検証 (実結果)

- 「違う」の件数: 18:25 **5件** → 19:15 **1件**。変種の判定が目視の前に落としている
- 新しい変種ルールで落ちるのは 3,767候補中 **23本**。全部 中身を見て別の刷りだと確認
  (途中で `(SP/SR)` を「SPだけ」と読む誤爆を2件見つけて直した)
- 目視待ち455本のうち **257本が値段なし**だった (mercari 223 / snkrdunk 34)
- 「違う」10件のうち **6件が名前も値段も空の候補**だった = 中身を見ずに永久NGにしていた
- 19:15 の走行: 15件確定・**51本追記**・入替22本 (今日最多)

### 監視くんとの連携 (3本)

- 補URLを消していたのは監視くん。売り切れ登録を実装してもらった
- 巡回を HIGH (出品済・6時間おき) と CAND (出品候補・1日2回) に分割
- **CAND を 16:00 / 05:00 に変更してもらった**。17時の出品時点で在庫情報が
  7時間前 → **30分前**になる

### 明日

1. 抽出くんのトレジャーハント フル走行 (BSOD の確認も兼ねる)
2. メモリ 24GB 化 (届いたら)
3. 見送りの記録が実際に効くか (次に見送りを押した走行で確認)

★BSOD は 9/18 16:28 を最後に **27時間 0件** (メモリ圧縮の停止が効いている)

---

## 2026-09-18 19:40 [HQ] テラピークが出品と仕入れの両方に繋がった / 「押しても減らない件数」2件を直した

### 決定

- **売れ筋 (テラピーク) の一致は product_id とカード番号の両方で見る**。
  KEY 未記入の候補が当たらず、今日の走行は「トレジャーハント 0件」だった
- **抽出くんのフル走行は明日以降**。夜は監視くんの巡回 (19:30 / 22:45 / 毎時の取りこぼし回収) が
  動くので、1〜2時間ぶつけない (ユーザー確定)
- **補充できない古い出品4件はサヨナラ** (ユーザー確定)。7〜8月に売れた SKU 旧形式の出品で、
  シートに最初から行が無く仕入元も分からない。誰かが消したのではない (2026-09-17 調査済)

### 変更 (commit)

| commit | 何を直したか |
|---|---|
| 3865726 | 売れ筋の一致をカード番号でも見る (PSA自動の並べ順) |
| 47025af | 目視画面: 写真が空の候補も「🛒仕入元」を出す (仕入元URLから引く) |
| bd4fcbf | 売れた分を補充: 補充した出品を live キャッシュにも反映 |
| 1a52e93 | Console 0.7.4: 上の段 (件数バー・3つの切替) を画面に固定 |
| 79ff0c3 | 売れた分を補充: 補充できない古い出品を対象から外す |
| f4383e5 | 棚②: 件数が「今日すでに落とした分」を引いていなかった / 取下げ分を live キャッシュから消す |

### 検証 (実結果)

- 売れ筋の突き合わせ: 一致する候補はスプシに **44件、うち40件は仕入元が売切**。
  生きている4件は全て KEY 空だった = 0件になっていた理由。テスト 5,171件 緑
- 目視画面: 写真が空の候補は 1,577件中 **77件**。cert 149501652 / 136006234 の
  仕入元画像が 200/image/jpeg で読めることを実機確認
- 「押しても件数が減らない」2件とも実機で確認: 補充 1件→0件 / 棚② 3件→0件。
  どちらも**走行は成功していて、結果がキャッシュに戻っていなかった**のが原因
- 入稿CSV (tcg_upload_20260918_183815.csv): 18行・エラー0・除外0。19→18 は
  仕入元URLが既存出品と重複した1行が自動で外れたもの (正常)
- 監視くんの定期: 7タスク全て Ready・直近の結果は全て 0 (成功)

### 残務の整理 (3件クローズ)

- №270 PSA の並べ順: 19件が16の弾に分散 (最大2件)・同じカードの重複0。9/17 の偏りは再発なし
- №258 夜間の UT 補URL: 83行中「拾えた0」は3行だけ・使えた18本。再仕入れも動作
- №231 どのシートにも無い出品: 22件の内訳は **公式サイト仕入5件 (正常)** と
  **取り下げ済み16件 (ファネルが古いだけ)**。落とさない判定は正しい

### 明日

1. 抽出くんのトレジャーハント フル走行 (BSOD の確認を兼ねる)
2. メモリ 24GB 化 (届いたら)

---

## 2026-09-18 18:20 [HQ] PC が1日6回落ちた — 原因はメモリ圧縮 / ログは「直前の1本だけ」に

### PC のクラッシュ (今日の主題)

9/16 から BSOD が急増 (9/16 2回 / 9/17 4回 / 9/18 6回)。**抽出くんを動かした時だけ**落ち、
最後は立ち上げただけで固まった。7/26 に1回あった後、8月〜9/15 の7週間は0回。

**効いた対策: メモリ圧縮の停止** (`Disable-MMAgent -MemoryCompression` + 再起動)。
16:28 を最後に落ちていない (18:20 時点で約2時間)。今日3回出た `0x154` は
メモリ圧縮の失敗そのものなので筋も通る。**この設定は戻さない。**

- メモリ増設: CFD `D4N3200CS-16G` 16GB を ¥10,980 で購入済。
  ASUS Vivobook M1502YA は 8GB 直付け + スロット1本なので **8+16=24GB** が上限。
  届いたら差し替え (残務 №282)。**抜いた 8GB は捨てない** — 再発時の切り分け材料
- 抽出くんは `--disable-gpu` を入れて33分1本 完走 (件数も落ちず: 45語1,340件 /
  Chrome 合計 3.36GB / 残留0)。ただし33分では確定と言えないのでフル走行が要る
- Windows Update は 9/25 まで停止中

**原因調査で3回外した** (メモリ不足 → 抽出くんの Chrome 残留 → KB5129195)。
毎回ユーザーの指摘で潰れた。「以前からしていることだ」「この4日で何かが変わった」
「抽出くんだけ落ちるのはおかしい」。**落ちた日を全部並べて、直前に入った物と突き合わせる**
のが最短だった。メモリ診断 (mdsched) は2回とも結果ログが残らず、2回やらせてしまった。以後使わない。

### 出品くん Console 0.7.1 → 0.7.3 — ログは「直前の1本だけ」

ユーザーがログをコピーして貼ろうとすると、走行が**3本ぶん**入っていた
(補URL③ 17:50 / 補URL③ 17:57 / 再仕入れ② 18:05)。

- 「ログをコピー」= **直前の1本だけ**コピーする
- 「🗑 ログを消す」= 前の走行ぶんを消して直前の1本だけ残す (1本なら全部消す・見出しも消す)
- 走行の境目は「▶ 」で始まる行
- 旧パネル (`control_panel.py`) 側も、走行開始時に自動でログ欄を空にするようにした

**ここで3回 噛み合わなかった。** ユーザーの「ログを消すボタンが欲しい」に対し、
私は (1) 既存ボタンの場所を案内 (2) 旧パネルに追加 — 動いていたのは Console だった
(3) 表示を全消し、と答えた。本当の要件は **「コピーした時に1本だけ入ること」**。
用途 (コピーして貼る) は最初に言われていたのに、手段 (ボタン) の話に寄せてしまった。

### 抽出くん

- `--disable-gpu` の1回走行は完走・件数維持 (`a045ac0`)。(2)(3)(4) は保留、次はフル走行
- トレジャーハントは実装中 (`run_harvest_mercari_treasure.py` 他)。深夜3時・
  タブ名 `mercari_psa10_treasure`

### 出品側

- トレジャーハントの出品優先は **HQ 側は完成済** (`5b19672`)。`demand_market.csv` 114種類を
  読んで並べ替える。抽出くんが止まっていても使える

### 次にやること

- 抽出くんにフル走行 (1〜2時間) を1本回してもらい、落ちないか見る
- メモリ到着後に 24GB 化 (残務 №282)
- PSA 再仕入れ (2) CSV が「生成対象0件」で returncode=1 (赤) になる。
  やることが無いだけなので正常終了にしたい (未着手・ユーザー判断待ち)
