...
## 2026-09-22 07:xx [Advisor] 補URL③の二重表示 / 放置の山を深堀 / 無断の値下げを止めた / V9 のミラー価格を実態に合わせた

### 決定 (ユーザー確定)

- **放置している山 (空振り・保留・「市場に無い」) は件数で済ませず、中身を並べて結論まで出す** (メモリ dig_into_parked_items)
- 補URLの番号なし検索は **絵柄1種類のカードだけ**。同名再録の混入は目視で弾く
- 絵柄の自動判定は 8/30 に停止済み (課金なし)。表示されている一致度は過去の記録の使い回し
- **値下げ印 (AL列) を全部消して V9 標準価格に戻す**。夜間処理は AL を書かない (印はボタンだけ)
- 利益計算の正本は **V9**。UK/AU/CA 計算の9行目は「US の式の出品価格 × 為替」= 実際のミラー価格

### 変更

- 補URL③: 候補を既存/違うを除いてから安い順に切る (382787be) / 押し出した・入らなかった候補を見送り記録 (8a930d16) /
  入れ替えの¥1,000判定は今の補URL5本の最高値と比べる (ce73458d) / 既存補URLの値段も見て並べ安い既存を捨てない (4900e645) /
  監視くんに個別確認した640本の値段を値段表へ (503894a4、値段不明 640→5本) / B列9999を対象外 (2fa4847d)
- 補URL検索: 検索語の名前を半角化・空白詰め (7738dbfa) / 絵柄1種類は毎回番号なしでも検索 + 名前照合で全角半角吸収 (df71c379)
- KEY: 目視で確定した版に毎晩合わせる (f269531a、71件修正。候補時点の仮KEYを上書きしない決まりが原因)
- 夜間: noconvert_pricedown は AL を書かない (372ade8f)。AL の印 HIGH259/LOW76 を消去 (控え iMak_data/hq/al_pricedown_flags_backup_20260922.json)
- 現在地: CSV監査HOLD は直近30日に無ければ1行 (8e73e7f4)
- シート: 別セットの補URL (ヒビキのホウオウex M2a を SV9a 出品の補に) を外した / サボ OP13-004 の2枚目2本を出品済みの補URLへ /
  PSA8 (チリ) と同じ itemID の重複行10組の売り切れ側を 9999 に
- V9: US計算 R25/R27/R28 を 1.00 (実測 AU947/UK958/CA937件の中央値 0.997〜1.000) / CA計算18行目の参照誤り (AU行) /
  UK/AU/CA 9行目を US 式×為替に・18行目消去・15/16行目の送料範囲44行目まで・AU L15 プロモ費。控え bk_*_20260922、変更履歴に記録
- 依頼: catalog `2026-09-21_don_character_and_mirror_variants` / `2026-09-22_psa_brand_set_code_table` (64種) /
  inventory `2026-09-21_hoju_price_recheck` (完了) / revise `2026-09-22_price_not_following_cost` (回答: AL値下げが原因)

### 検証

- 補URL③: ヨマワルAR・ミュウツーの実検索で既存補URLと安い新規候補が番号未確認枠に出た。入れ替え対象 5→2件 (残りは実際に¥1,000以上安い)
- V9 9行目: UK £100.95 / AU A$171.31 / CA C$738.00 が独立計算と一致。offer_calc の照合対象 (16行目) は値不変
- 値下げ印の効果: 印の付いた332件で売れたのは1件 (ヤドン SM-P 253)
- pre-commit 全テスト緑 (5,4xx passed)

### 次にやること

- 明朝 04:30 のリバイスで AL 解除分 335件が V9 標準価格に戻ったかを確認 (例 358704538293 → $103.98 / AU A$146.01)
- catalog の回答待ち2件 (ドンカードのキャラ名・ミラー版 / PSAセット名表) → 表ができたら psa_to_csv に読む口 (残務)
- UT 補URL③ にも同じ「候補が隠れる」問題があるか未確認
- 同じ itemID の重複行2組 (孫悟天・キッド＆キラー) はどちらの仕入元が生きているか未判定

---

## 2026-09-21 19:xx [HQ] リサーチの台帳を取り直した (市場の2割しか見ていなかった) / トレジャー1,059行を HIGH へ

### 決定 (ユーザー確定)

- **テラピークの検索語は `PSA10` と `PSA 10` の2本**。`PSA10` だけだと空白ありの出品がほぼ抜ける
  (台帳1,970件中 `PSA 10` だけの表記は1件 / うちの売れた19件は全部 `PSA 10` で台帳に0件)。
  2本は別の出品を拾う (`PSA 10` 側に `PSA10` 表記は126件だけ) ので両方要る
- **価格は $50〜$760** (上限 = 会社の仕入上限7万円を v9 で出品価格にした値)。**既定の期間は30日**
  (90日だと `PSA 10` のポケモンが1検索1万件の上限を超えた)。今回の90日分はそのまま使う
- **日本のセラーに絞らない** (セラー国は GetItem 約9,000回 = 1時間超・枠をほぼ使い切る。「価値なし」)。
  代わりにタイトルの言語の印 (英語版/中国語版/韓国語版) で落とす
- 値付けは どの経路も **V9 の cost-plus だけ**。市場の値段は使わない
- タイトルは直さない (出品中605件とも既に `PSA 10`。買い手の検索は空白を区別しない = 実機で確認)
- **トレジャーの候補は 仕入7万円以下を全部 HIGH へ** (◎△を問わない)。重複は足さない・足した行は緑

### 変更

- `market_ledger.py` 検索語2本 / 価格 $50〜$760 / 既定30日 / 同じ出品を1行にまとめる鍵 /
  `JP_SELLER_ONLY=False` / archive の退避名を指定可に。`console/server.py` は検索語ごとにタブを開く
- `terapeak_grab` 上限 50→200ページ・打ち止めでも CSV を出す・unlimitedStorage (v1.2)
- 新規 `chara_demand_xlsx.py` (キャラ×版 の需要表・Excel) / `chara_targets_build.py` (探す先 = 市場キャラ+うちのキャラ)
- 新規 `treasure_to_high.py` (中間スプシ → HIGH・緑塗り)。**HIGH 1,059行 追加** (N列 #REF 0 を確認)
- `sheet_io.append_product_rows` に **1点ものの仕入元が HIGH に在れば足さない** 守り
  (監視くん指摘: 出品中と同じスニダン/メルカリ URL の候補2行。HIGH の重複6組も片付けて A列重複0組)
- `iMakTCG/check_csv.py` **`Gem Mint` を禁止語から外す** (9/20 の決定 4bf9e5cb で生成側が入れるのに、
  監査が直し忘れていて3件除外)。Console を 18:05 前の判定のまま動かしていたので再起動済
- `auto_catalog_add_request` 「カタログに在るのに落とした件」を層 `code` に (catalog に送らない)。
  既存4行も書き換え。ログの「該当なし (カタログに依頼)」→「(目視で当てはまる候補が無かった)」
- 台帳 1,970 → **11,231件**。№1 未出品609種類 / №2 5枚以上キャラ177 / №3 うちのキャラ17 (重なり10)。
  トレジャーの出品待ち 28種類 (そのまま利益7)。図: デスクトップ `リサーチ3分類.pptx` (2枚)

### 依頼 (返事待ち)

- catalog `2026-09-21_kihada_name_en_katy.md` キハダ5件の英語名が Katy (正しくは Dendra。Bulbapedia 取り直し済)
- catalog `2026-09-21_pokemon_name_en_blank_bel.md` (監査くん Act) ベルのまごころ/ネルケ の英語名が空
- harvest `2026-09-21_chara_keywords_draft.md` **ユーザー指示で保留** (184キャラ。出すのはまだ)

### 次

- ACTIVE を取る (本命 A = 売れた×今誰も出していない を №1〜3 から抜くため)。検索API で約600回
- 再出品の行を作る `psa_resource_gate.py` が出品中と同じ仕入元を使った件 (守りは入れたので発生源の確認のみ)

---

## 2026-09-20 21:xx [Advisor] 「市場実績で絞る門」は入れない — 売れた15本に当てたら1本も通らなかった

### 決定

**市場SOLD を出品の門にしない。** トレジャーハントの停止 (同日 HQ 記入) と同じ結論に、
別ルート (うちの販売実績との突合) からも到達した。

### 検証 (実データ・全部 2026-09-20 取得)

うちの live PSA10 (US) 605本、90日で売れた15本に、作りかけた門を当てた結果:

| | 件数 |
|---|--:|
| 市場台帳 (JPセラー PSA10 90日・1,970出品) に在った | **2本** |
| 台帳に無い (90日間 誰もそのカードを売っていない) | **13本** |
| 台帳に在った2本のうち 実売最高値を超える値段だった | **2本とも** |

- $517 ゲンガー&ミミッキュ ← 市場の実売最高 $395
- $87 バリヤード ← 市場の実売最高 $42

**門を入れていたら、売れた15本を1本も出品していない。**
...
|---|--:|--:|
| 1日あたり表示 | 93 | 90 |
| 同カードのACTIVE数 (番号一致) | 14本 | 12本 |
| うちが最安だった率 | 0% | 3% |
| 価格帯別の売れた率 | $75-125 2.5% / $125-200 2.5% / $200-400 3.1% / $400- 2.0% |

- **うちの価格は同カード最安の 1.79倍**。値段が判明した8件は**全部オファー成立** (定価0件、値引き中央 -23%)
- 掲載日数: 売れた分 中央 **78日** / 売れていない分 30日
- 年代だけ差が出た (SM期 7.4% / SV期 2.8% / 剣盾 0%) が **n=29** と小さく、価格を揃えると
  $150-300帯で 20.0% 対 2.4%。**判断材料にはするが門にはしない**

### 変更 (道具のみ。判定ロジックは触っていない)

- `iMakHQ/tools/our_sold_fetch.py` — うちの SOLD を eBay GetOrders から取る (90日を30日×3回)
- `iMakHQ/tools/demand_table_build.py` — 台帳 → カード単位の需要表 676種類
  (`market_sold/demand_full.csv`)。`demand_market.csv` は83枚しか無く市場の1割だった
- commit `b22c9468` / `a266f6a6`、push 済み。テスト 5,360 pass

### 次 (ユーザー宿題・未着手)

戦略「誰も出していないカードを、誰も付けていない高値で並べて待つ」を分解して仮説化する:
① 誰も出していないカード = SOLD と ACTIVE の組み合わせロジック /
② 誰も付けていない高値 = コストプラス2 を入れるか /
③ 待つ = いつまで (実測の中央は78日)

---

## 2026-09-20 [HQ] トレジャーハント初走行 → **方針を見直して停止**。狙う所を決め直す

### 結論 (ここだけ読めば分かる)

**「市場で売れているカードを探す」という方針に、根本的な疑問が出た。** 夜間走行は止めた。

- 今日 集めた221件 (売れ筋に当たる分) のうち、**そのまま仕入れて利益が出るのは17件 (8%)**。
  メルカリの相場は eBay 実売から逆算した上限の **中央値1.7倍**
- 売れ筋は競合が多く、**無在庫のうちは価格で負ける**。国内相場に利益と送料が乗るため
- 狙うべきは「売れている」より **「今 誰も出していない」** かもしれない
- それを判定するには **ACTIVE (今 出ている物)** が要るが、まだ取っていない

★「うちが売った28件が市場の売れ筋115種類と1件も重ならない」= **意味のある結果ではない**。
  母数が小さすぎて、偶然の重なりは1〜2カードしか期待できない。一度そう読んだが撤回した。

### 次にやること (この順)

1. **ACTIVE を取る** (Research の「出ている」タブ。道具は対応済み)
2. **うちの SOLD 実績を注文 API から正確に取る** (`sold_restock.orders_from_api`)
3. 「売れている × 競合が居ない」の4象限で狙う所を決める → 抽出くんに再依頼

| ACTIVE | SOLD | 意味 |
|---|---|---|
| あり | あり | 激戦区。価格で負ける |
| あり | なし | 他社は出しているが売れていない (出したてかも。読みにくい) |
| **なし** | **あり** | **理想。需要あり・競合ゼロ** |
| なし | なし | 未知 |

### トレジャーハント初走行 (抽出くん)

7時間17分・3,097件から **1,129件をシートに追加**。BSOD 無し (空きメモリ最低3.6GB)。

★**使った一覧 (9/18版) が間違っていた。** 売れ筋を **買い手の国**で絞って取っていたため
  (下記)。古い一覧114件のうち **48件は探す必要が無かった**。新しい一覧との重なりは66件。

HQ 側で整理した:
- **KEY を 648件に埋めた** (`treasure_fill_key.py`)。引けない481件は **番号がタイトルに無い**
  のが448件。メルカリは番号を書かないことが多く、引き方を直しても8件しか増えない
- AL列「トレジャー」に印を付けた → **◎ 出せる 58件 / △ 高い 163件 / 印なし 908件**
- 門 (ユーザー確定): `上限仕入れ値 +¥7,000` かつ `×1.5`、**かつ 会社の上限 ¥70,000**。
  きつい方が効く。幅を持たせるのは 値引き / ライバルの売り切れ / 補URL の入れ替えを見込むため
- ★**◎ 58件の HIGH への移動は保留**。狙う所が決まるまで出さない

### 売れ筋データの条件が間違っていた (真因)

**eBay の Research は `sellerCountry` を見ていない。** 9/18 の1,612件は **買い手の国**で
絞られており、米国セラーの英語版が309件 混ざっていた。ユーザーに「8は英語版」
「77,78,79 英語版」「162〜165,173〜175 英語版」と何度も指摘させた。

- 名前を `sellerCountry` に直して取り直したが、**それでも JP 896 / US 892 とほぼ半々**。
  入口では絞れない → **出口 (GetItem のセラー国) で落とす**ようにした
- 言語も **申告値 (GetItem の Language)** で見る。タイトルの文字では11件中4件しか見分けられない
- 日本セラーだけにすると 175種類 → **115種類**。値上げできる出品は **実質ゼロ**
  (前に「5件ある」と出したのは、英語版や別の刷りと比べていたため)

### カタログの引き方を1本にまとめた (`tools/catalog_lookup.py`)

ユーザー「カタログの引き方に関して、ノウハウを蓄積しておかないとね」。

**「カタログ要補充 17件」と言っていたが、16件はカタログに在った** (①ではなく②)。

    17 → 7  カード番号そのもの (specs の card_number_text) で引く
     7 → 2  **英語のカード名**で決める (市場のタイトルは英語。name_en は 22,435件)
     2 → 1  eBay ライブ配信の日付 (`07/25`) を番号と読まない

落とし穴は docstring とテスト14件に残した。`python catalog_lookup.py "<タイトル>"` で
①データが無いのか②引き方が悪いのかを その場で判定できる。

あわせて **画像が英語版だった**のも直した (ワンピースは先頭が英語版 4,760件のうち
4,591件が2枚目に日本語版を持つ)。ポケモンは全部日本語版なので気づけていなかった。

### 競合の出品を丸ごと取った (GetItem 1,515件)

ユーザー「丸ごとなら、取り直し要らないね」。**値段の比較だけでなく、出品を強くするため**。
タイトル / Item Specifics 全項目 / 発送・返品の条件 / 写真枚数 / セラーID・評価・国 / 売れた数。

分かったこと (まだ手を付けていない):
- **発送までの日数: 市場の中央値3日 / うち7日** ← 検索順位に効く
- Best Offer 972件 (63%) が受付 / 送料無料 640件 / 返品 714件 (30日が506件)
- うちは Item Specifics 25項目中22を埋めていて、競合との差は5項目前後

### 2つ目の API 鍵 (imaktrading)

作成 → 「Marketplace Account Deletion の免除」を申請 → 有効化まで完了。
GetItem が **5,000 → 10,000回/日**。市場調査はこちらで走らせ、本番の枠を減らさない。

★新しい鍵は作った直後は使えない。developer.ebay.com → その鍵の `exemption` から
  「I do not persist eBay data」を選ぶ (手順は `iMak_data/credentials/README.md`)。

### 監視くん

- 巡回を HIGH (出品済・6時間おき) と CAND (出品候補・1日2回) に分割。
- **CAND を 16:00 / 05:00 に変更してもらった** → 17時の出品時点で在庫情報が
  7時間前 → **30分前**になる (eBay の日付は日本時間17時以降なら通年で新しい日)

### 明日

1. ACTIVE を取る → 4象限で狙う所を決める
2. うちの SOLD 実績を注文 API から取る
3. 決まったら抽出くんに再依頼 (夜間は止めてある)

★メモリ 24GB 化はまだ (残務 №282)。BSOD は 9/18 16:28 を最後に **2日間ゼロ**
