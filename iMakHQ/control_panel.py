...
SCRIPTS = [
    ...
    },
    {
        ...
        "category": None, "type": "utility",
        # ★2026-09-16 ユーザー「ラベルの意味が違うよね」: この画面は新品の特定だけでなく、
        #   **出品済みで KEY が無い行の KEY 埋め**も、出品待ちの行も、新しい候補も全部出す
        #   (KEY 埋めが先)。🤖自動 側の目視は新しい候補だけ (--new-only)。名前を実態に合わせた。
        "label": "🩹 UT 目視特定 (KEY埋め+候補)",
        "badge": "ut_identify",
        "tip": "UT の目視を **まとめて** 見る画面 (KEY 埋めが先 → 出品待ち → 新しい候補)。"
               "出品済みの行は KEY を入れるのが目的で、売り切れ・中古でも商品を決める。"
               "出品を増やすだけなら 🤖自動 の目視で足ります。確信が無ければ選ばない。",
        "cwd": f"{WORKSPACE}/iMakHQ/tools",
    ...
    },
    {
        ...
        "tip": "在庫はあるが売れていない出品を落とします。**売れるかもしれない物を"
               ...
               "押してください。条件は 一度も売れていない / 出品から30日超 (TCG・G-SHOCK・"
               "メルカリ/ラクマ仕入の Tシャツ) / US出品。有在庫と公式仕入の Tシャツは落としません。",
    },
    ]
    ...
    def refresh_hoju_badge(self):
        ...
        try:
            ...
            if nc.get("error"):
                ...
                # ★2026-09-15: 数には「まだ売り切れと判っていない」候補が入る (ページを開かないと判らない)。
                #   押すと在庫を確かめて **買える分だけ** 20件そろえて出し、売り切れは台帳から消える
                n_txt += "\n(売り切れを含む数。押すと在庫を確かめて買える分だけ出します)"
                if nc.get("auto"):
            ...
            if pg.get("error"):
                ...
                # ★2026-09-10: 候補が全部「違う」判定済 = 押しても画面に出ない札。
                #   新しい仕入元が出るまで人には何もできないので、残数から外した上で
                #   「消えたわけではない」ことだけ書く。
                if pg.get("ng_blocked"):
                    pg_txt += ("%s※候補が全部「違う」判定済 %s件 "
                               "(新しい仕入元が出るまで出てきません)"
                               % (chr(10), pg["ng_blocked"]))
