...
def test_qty_zero_kept():
    picked = _items("1", "2")
    kept, revived, failed = cull_end.verify_oos(picked, lambda i: 0)
...
def test_qty_positive_excluded():
    ...
    kept, revived, failed = cull_end.verify_oos(picked, fetch)
...
def test_qty_none_excluded_failclosed():
    picked = _items("1", "2")
    kept, revived, failed = cull_end.verify_oos(picked, lambda i: None)
    assert not kept
...
def test_fetch_exception_excluded_failclosed():
    ...
    picked = _items("1")
    kept, revived, failed = cull_end.verify_oos(picked, boom)
    assert not kept
