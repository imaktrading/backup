...
def test_out_of_stock_is_first():
    assert SE.tier_of(_row(qty=0)) == SE.TIER_OOS


def test_old_and_unsold_is_second():
    assert SE.tier_of(_row(age=200, sold=0)) == SE.TIER_STALE


def test_new_listing_is_left_alone():
    """出品30日未満はまだ判定できない。"""
    assert SE.tier_of(_row(age=10)) is None


def test_sold_is_left_alone():
    assert SE.tier_of(_row(sold=1)) is None
    assert SE.tier_of(_row(s90=1)) is None


def test_out_of_stock_wins_even_with_demand():
    """在庫切れは買えないので、watcher が居ても売れた実績があっても落とす側。"""
    assert SE.tier_of(_row(qty=0, watch=9, sold=3, age=5)) == SE.TIER_OOS
...
def test_watcher_alone_does_not_protect():
    ...
    """
    assert SE.tier_of(_row(watch=5, sold=0, age=200)) == SE.TIER_STALE
...
def test_second_tier_orders_by_access_ascending():
    ...
    picked, _ = SE.pick([seen_a_lot, seen_little], target=50, shelf_of=_shelf)
...
def test_out_of_stock_beats_everything():
    ...
    picked, _ = SE.pick([stale, oos], target=5, shelf_of=_shelf)
