#!/usr/bin/env python3
"""
iMak Trading Japan - eBay CSV チェッカー (Mercari/Porter)
...
def search_ebay_active(token, keywords, limit=50):
    ...
    params = {
        ...
        "filter": (
            ...
            "categoryIds:{57988|52357|11450|15687}"  # Bags, Clothing, T-shirts
        ),
        "sort": "price",
        "limit": min(limit, 200),
    }
...
EBAY_SPEC_TO_CSV = {
    "Brand": "C:Brand", "Type": "C:Type", "Size": "C:Size",
    "Color": "C:Color", "Department": "C:Department",
    "Style": "C:Style", "Material": "C:Material",
    "Pattern": "C:Pattern", "Features": "C:Features",
    "Closure": "C:Closure",
}
...
def build_search_query(row):
    ...
    character_family = get_col(row, "C:Character Family")
    color = get_col(row, "C:Color")

    parts = [brand, item_type]
    if character_family:
        parts.append(character_family)
    if color:
        parts.append(color)
...
def validate_row(row, row_idx):
    ...
    if category not in ("57988", "52357", "11450", "15687"):
        issues.append(("ERROR", f"カテゴリが不正: {category}"))
...
def claude_review(rows, all_issues, all_comp_findings, all_gates):
    ...
    for i, row in enumerate(rows):
        ...
        brand = get_col(row, "C:Brand")
        item_type = get_col(row, "C:Type")
        size = get_col(row, "C:Size")
        color = get_col(row, "C:Color")
        department = get_col(row, "C:Department")
        style = get_col(row, "C:Style")
        ...
        summary_lines.append(
            f"#{i+1} Title: {title}\n"
            f"   Price: ${price} | Brand: {brand} | Type: {item_type} | Size: {size}\n"
            f"   Color: {color} | Department: {department} | Style: {style}\n"
            f"   {issues_text}{comp_text}{gate_text}"
        )

    prompt = f"""You are an eBay listing quality reviewer for Japanese used fashion items (Porter bags, clothing, etc.).
...
# ===== メイン =====
def main():
    print("=== iMak Trading Japan - eBay CSV チェッカー (Mercari/Porter) ===\n")
