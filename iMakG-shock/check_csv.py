#!/usr/bin/env python3
"""
iMak Trading Japan - eBay CSV チェッカー (G-SHOCK)
...
#   旧: ebay_fee_rate=0.185 等を直接記述 → 利益計算シート v2 / yaml と乖離
#   新: profit_params 経由で yaml(SSOT) から取得、カテゴリ名のみ注入
...
PROFIT_PARAMS = _gccp_pp("G-SHOCK")
...
def search_ebay_active(token, keywords, limit=50):
    ...
    params = {
        ...
        "filter": (
            ...
            "categoryIds:{31387}"  # Wristwatches
        ),
        "sort": "price",
        "limit": min(limit, 200),
    }
...
EBAY_SPEC_TO_CSV = {
    ...
    "Color": "C:Color", "Movement": "C:Movement",
    "Case Size": "C:Case Size", "Band Color": "C:Band Color",
    "Band Material": "C:Band Material", "Display": "C:Display",
    "Features": "C:Features",
}
...
def validate_row(row, row_idx):
    ...
    # --- カテゴリ・条件 ---
    if category != "31387":
        issues.append(("ERROR", f"カテゴリが 31387 でない: {category}"))
    if condition != "1000":
...
def claude_review(rows, all_issues, all_comp_findings, all_gates):
    ...
    for i, row in enumerate(rows):
        ...
        model = get_col(row, "C:Model")
        color = get_col(row, "C:Color")
        movement = get_col(row, "C:Movement")
        watch_type = get_col(row, "C:Type")
        case_size = get_col(row, "C:Case Size")
        ...
        summary_lines.append(
            f"#{i+1} Title: {title}\n"
            f"   Price: ${price} | Brand: {brand} | Model: {model} | Color: {color}\n"
            f"   Movement: {movement} | Type: {watch_type} | Case Size: {case_size}\n"
            f"   {issues_text}{comp_text}{gate_text}"
        )

    prompt = f"""You are an eBay listing quality reviewer for Japanese G-SHOCK watches.
...
Rules to enforce:
- No forbidden words: Japanese, GEM MT, Japan, Mint, L@@K
...
# ===== メイン =====
def main():
    print("=== iMak Trading Japan - eBay CSV チェッカー (G-SHOCK) ===\n")
