...
- URL: https://www.ebay.com/itm/<itemID>  (200 OK で取得可、tools/verify_itm_url_diag.py で確認済 2026-04-30)
- 旧仕様 doc は「公開 itm URL は 403 → seller hub 経由」と書いていたが、
  実際は profile 認証 driver で 200 OK + 数 MB HTML 取得可。seller hub 切替不要。
- qty 判定: _detect_qty_state() で複数シグナル fall-through
...
def _detect_qty_state(html: str) -> tuple[str, str]:
    ...
    判定優先度:
      1. ended (listing has ended)
      2. qty_zero (Out of stock badge / 在庫切れ JP)
      3. 数値 availability (N available / N in stock / N left)  ← 主シグナル
      4. Add to cart button id (構造シグナル)
      5. unknown (シグナル無し)

    注意: 過去あった "Limited stock" 単独マッチ / "Only ... left" 単独マッチは
    eBay UI 上の無関係箇所 (sidebar / search / promo 等) で頻繁に false positive
    を生むため除外。数値付きパターンのみ許容する。
    """
    h = html
    # 1. 取り下げ済 / 終了
