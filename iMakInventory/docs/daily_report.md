...
## 2026-09-19〜09-21 — 巡回の分割を実測で確認 / 補URL台帳 / 候補の大量追加

### 巡回の分割 (9/19 実装) の結果
- HIGH (出品済だけ): **3.3時間 → 1時間50分**、完走。CAND (候補だけ): 35分
- 出品くん依頼で CAND を **16:00 / 05:00** に変更 (17時の出品直前に鮮度を合わせる)。
  04:00 は日次バックアップ (eBay ブラウザ設定をコピー) と重なるので避けた
- **候補を1,022行追加** (1,992 → 3,014行、見に行く数 560 → 1,562件)。CAND は約1時間45分に
  伸びる見込みで、16:00 開始だと17時に間に合わない。**ユーザー判断: 一晩様子見**
  (明朝 05:00 が追加分込みの初回。所要を見て 開始繰上げ or 専用ブラウザ設定で並行、を決める)

### 補URL (出品くん依頼 2本)
- 売切で消した1点ものの補URLを `hq/not_buyable_urls.json` に登録 (`fd30dcb`)。過去分込み 356→2,626件
- 再入荷しうるもの (メルカリShops / ヨドバシ) は `hq/not_buyable_restockable.json` に分離、
  在庫が戻ったら自動で消す (`a12fbf8`)。のべ1,185件 = ユニーク283件。初日に21件消去
- スニダン補URL 1,066本から40本を実地確認: 買える39 / 売切1 (前回巡回後に売れた分) / 判定不能0

### 仕様変更で検知できなくなっていないか (9/21 点検)
- メルカリ 判定不能0.5〜1.5% / 売切検知 約20%、Amazon 判定不能0.5〜1% / 約36%。1週間横ばい
- ラクマが8日間売切0件 → 実地で確認、生きている12件は全て在庫あり・過去の売切8件は正しく売切。
  対象が20行と少ないだけ
- **仕様変更検知の自動化は未実施** (ユーザー未回答)

### 自分のミス
- 走行中の本番巡回と同じブラウザ設定で dry-run を走らせ、**HIGH に39行の判定不能を出した**
  (9/19 08:18〜09:37)。fail-closed で誤取下げなし。以後 走行中は実データで scraper を動かさない

### シート点検 (9/21)
- 仕入元URLの重複6件 → HQ が全て削除、残り0件。うち2件は出品済と同じ1点もの仕入元の候補
  (出品すると1つの現物に出品2つ)。HQ が行追加の入口で同じ仕入元を弾く再発防止を入れた
- A列空の7行は有在庫分 (巡回対象外で正しい、ユーザー確認)

## 2026-09-19 — 巡回が長い / 完走しない の切り分け

- **巡回時間の伸びは行数増**が原因 (7月1,392行0.9h → 9月2,816行3.3h、1行4秒は不変)。
  実際に見に行くのは1,262件で、うち **560件 (45%) が itemID 空の出品候補**だった
- **出品済と候補を別の巡回に分けた** (`a6b9805`、ユーザー判断)。HIGH は出品済だけ(702件)、
  候補は `CAND` で1日2回 (10:00 / 22:00、`iMakInventory_Cycle_CAND`)。候補は eBay に
  出ていない = 取下げ対象が無く、CAND が止まっても fail-OPEN にならない。
  itemID が入れば次回から自動で HIGH 側に戻る (行の移動不要)
- **落ちた理由が残らない問題を修正** (同 commit)。タスクは pythonw 起動で stderr が捨てられ、
  09-11 以降 少なくとも5回、ログが行の途中で終わっていた (9/16 は1,504行目で消滅)。
  例外と faulthandler を `logs/cycle_crash.log` に残す。**原因はまだ不明、記録待ち**
- 補URL救済ログのタブが1000行で満杯 → 8/17以降スプシに記録が残っていなかったのを修正 (`43ef792`)
- 巡回除外の現行基準: D=○ かつ 1点もの (メルカリ個人/ラクマ/スニダン/ヤフオク/PayPayフリマ)。
  今朝の実績 2,816行中 850行スキップ

## 2026-09-03〜09-17 — 監視の穴を塞ぐ (枠消滅 / 行増殖 / K列) + 巡回の安定化 + 通知の整理

### 取下げ漏れ・監視の穴 (公式監視くん)
- **仕入元から枠が消えると ◎ のまま凍る穴**を修正 (`a2cc513` `4f3fb40`)。
  色ごと消滅 / 単品の eBay サイズが仕入元に無い / 多色ページで1色だけ消滅 の3型。
  実害: 買えないのに生きていた出品 3件 (最長 45日、プラズマ1000 青)。3件とも数量0にして eBay で確認済
  - 初版は「同じ itemID に2商品URL」の出品で誤検知 → 根拠がある枠だけ拾う形に絞り、全107出品で検出3件・誤検知0
- **SKU詳細の行が毎 cycle 増殖** (単品の突合外れで append) を修正 + 157行削除 (`d42b7b6` `3eff5c4`)
- **K列(eBay現Qty)が実eBayとずれる**: 同期ツールが日次に入っていなかった。組込 + 下げ方向は対処済行も反映 → 218行修正、「仕入元✕×eBay在庫あり」225→7行 (`3eff5c4`)
- **pytest が二段確認 state を空で上書き**していたのを修正、消えた基準はシートから再構成 (`b501e39`)
- itemID 誤紐付け2行 (キングダム グレー / EVANGELION OFF WHITE) をユーザー確認のうえ空欄化

### 補URL
- 消す前にもう一度引いて確認 (`2e8bacf`)。掃除の上限を撤廃 (ユーザー判断 `e3e663f`)、急増ガードは補URL総数の25%連動 (`d491993`)
- 夜間補充の 64% が追加時点で売切 → HQ に依頼、HQ が書込前確認を実装 (`622e6f1`)

### 巡回の安定化
- HIGH を 4h→6h 間隔 (ユーザー判断 `241f9db`)。仕入候補+486行で1回が最長4.8hに伸びていた
- Chrome 掃除がプロセス一覧の読込失敗で丸ごと飛ぶバグ修正 + HIGH 実行上限 6h→11h (`b8d19d5`)
- **取下げ待ちを毎時05分に自動送信**するタスク追加 (ユーザー承認 `02bfcf1`)。巡回が PC 再起動等で途中で消えても最大1hで送る
- 実測: 09-09 / 09-13 / 09-14 / 09-16 に PC 再起動で巡回が消滅 (手動再起動・Windows Update・予期しないシャットダウン)。
  送れずに残っていた取下げ 9件 (09-13) / 5件 (09-16) を手動送信、eBay で残り0確認

### 通知
- ユーザー指示「意味のないアラートは出さない」: 補URL急増HOLD の ALERT / [SKIP]取下げ対象なしメール / 完走直後の誤「巡回停止」を停止 (`0218905` `985388f`)。[OK]実行報告メールは継続

### その他
- 復活前の採算チェックを削除 (監視くんは価格を持たない / 古い pricing_engine が V9 とずれていた。ユーザー判断 `10b7924`)
- 商品管理シート N/AN 列への書込経路を塞ぐ + pre-commit で検出 (HQ窓口依頼 `5a29092` `58094b3`)
- 棚卸レポートが終了済みを候補に出し続けていたのを修正 (`2c6855f`)。自動End(週次)は現状維持
- 依頼回答: Revise (variation/single UP の eBay 生文言、価格追随) / HQ (「84件の行が消えた」→ 行は残っており itemID だけ 09-15 に空にされていた。監視くんは B列を書かない)

## 2026-09-01 — 公式在庫の棚卸を自動化 (在庫0継続 → 自動 End + 履歴)

### きっかけ (ユーザー指摘)
- 「ユニクロには在庫あるのに eBay が売り切れ」(itemID 358793897931 / モンチッチ UT)
- 調査結果: **監視は正しかった**。出品しているのは JP S/M/L/XL で全て在庫0。
  買えるのは JP XS のみ (11点 ¥500、拡張サイズ枠 = 別 price-group) で、こちらは未出品。
  画面のサイズ選択が「斜線=終了 / 枠のみ=在庫なし(再入荷待ち)」で紛らわしく、
  スクリーンショットで「在庫なし・再入荷通知を登録する」を確認して決着。
- 横断確認: 公式シートの UNIQLO 105行を実測。「監視している枠に在庫があるのに売切扱い」= **0件**。
  「未出品の枠に在庫あり」= この1件のみ → HQ に情報連携 (`hq/requests/2026-08-30_uniqlo_xs_size_not_listed.md`)

### 判断の物差し (ログから毎回計算。固定値を書かない)
- 「在庫0 → 復活」の実績 318回: **302回 (95%) が 7日以内**、最長 39日、中央値 0日
- → **30日を超えたら まず戻らない**

### 実装
- 新設 `iMakeBayAPI/inventory_monitor/stale_zero_report.py` (test 15本)
  - 在庫0の継続日数を listing ごとに算出 (途中で1日でも戻れば日数リセット)
  - 週次タスク `iMakInventory_StaleZeroReport_Weekly` (月 07:30) で **30日以上を自動 End**
  - 履歴 `logs/ended_listings.jsonl`: いつ/何を/ブランド/商品名/いつから0/何日/理由/
    **仕入元URL**/終了後の実状態 → 再出品に必要な情報を全部残す
  - End 後に GetItem で Completed/Ended を確認した物だけ「完了」に数える
  - 終了した行はシート FLG=1 (巡回対象外)。行は消さない
- 安全装置
  - **ミラー除外**: End 前に site を確認、US 以外 / 読めない場合は触らない (fail-closed)。
    全社ルール (2026-08-24 eBaymag ミラー 643件 無駄打ち) に準拠。ユーザー指摘で追加
  - **急増ガード**: 1回 30件超は 1件も End しない (巡回側の誤判定疑い。End は不可逆)
  - 既に終了済には送らない

### 初回実行 (実測)
- 在庫0継続 45件 (30日以上 23 / 14〜29日 8 / 13日以内 14)
- **23件を End、23/23 の終了を実確認**、履歴 23件、シート 23行を除外
- 最長 106日 (ポケモン UT / 5-18 から在庫0)。ミラーは 0件 (23件とも site=US・自社)

## 2026-08-30 — 監査の中断を修正 + 取下げ漏れ3件を閉鎖

### reverse_audit が `sheet_read_failed: LOW APIError` で中断 (08-20 にも同型)
- スプシ読込を 1回の失敗で諦めていた → 15/30秒空けて3回試すよう修正。全滅時のみ中断
- 記録に例外の中身を残す (従来は型名のみで 429 か 5xx か判らなかった)

### 再実行で本物の取下げ漏れ 3件
- 乖離4件。うち1件は承認済 (物理在庫保有 row728)
- 残り3件は仕入元 mercari を実確認して全て売切、eBay は US 出品で qty=1 → 取下げ + 残数0確認
- **これらは 08-29 の監査でも同じく検出されていたが、誰も対応せず丸1日以上放置**
- → 監査が見つけた未承認乖離を **取下げキューに積む** よう変更 (送信は巡回の既存経路)。
  10件超は積まずに人へ回す (誤った一括取下げを避ける)

### test が本番の取下げキューを汚していた
- item_id="a"/"b"/"new1" 等が本番 pending_revise に混入 → eBay 照会が必ず失敗し、
  取下げツールが恒久的に「未完了あり」= **警告が嘘をつく**状態
- pytest 実行中は `*_TESTRUN.jsonl` に書くよう構造的に分離。混入5件は証跡付きで退避
...
- 決定: 08-11 に conflict として保留した 13 件のうち **8 件は eBay qty>0 (= 買える状態)**。
  「矛盾だから保留」は往復を止めるが、**保留の中に取下げ漏れが居座る**ことに気づけていなかった。
  シートの両論を信じず **仕入元を直接再取得**して決着させた。
- 実測と処置:
  - **gu EVANGELION `358711287999`**: eBay タイトルが "…**Navy** NWT" = NAVY 出品。
    gu 実データで **NAVY の XXL / 3XL は売切**。eBay は qty=1 = 買える → **本物の取下げ漏れ**。
    **US XL(JP XXL) / US 2XL(JP 3XL) を qty=0 に (GetItem verify 済)**。
    ※ 同 listing に OFF WHITE 行 (◎) が紐付いていたため矛盾扱いになっていた = **色違い行の誤登録**。
  - **montbell ウインドブラスト 6 件**: 商品ページに **"L" と "L-R" が両方実在するサイズ** (56 SKU)。
    L=売切だが **L-R は在庫あり** → **履行可能なので live のままが正しい** (取下げ不要)。
    = 「重複行」ではなく **eBay 1 variation ↔ 仕入元 2 SKU (丈違い) の対応が未定**が真因。
- 教訓: **保留 (HOLD) は「安全」を意味しない。** 保留の中で eBay qty>0 のものは
  取下げ漏れ候補として別カウントし、警告を出す必要がある (次の改修対象)。

### ② eBay 終了済 listing の行を バックアップ後に一括削除 (user 指示)
- 背景: LOW row522 (`357026358239`) が Amazon URL 死亡で `⚠ scraper ×47` を出し続けていた。
  GetItem は **ErrorCode 17 (Item cannot be accessed) = 終了済**。**出品も仕入元も両方死んでいた行**。
  user がシートから削除 → 同型の行を洗い出したところ **HIGH 117 / LOW 323 = 440 行**。
- 変更 [tools/purge_ended_listing_rows.py](../tools/purge_ended_listing_rows.py) 新設:
  - **1 行ずつ GetItem で実確認**し、**Active が 1 件でもあれば削除対象から外す**
    (active report に載らないだけの生きた出品を消すと監視の穴 = fail-OPEN を作るため)
  - 削除前に **同 spreadsheet 内のバックアップタブへ全列コピー** (元行番号・削除時刻・eBay状態付き)。
    **退避行数が対象数に届かなければ削除しない**
  - 削除は **行番号の大きい順** + **1 回の batch_update にまとめる** (429 対策) + 429 は 60s×3 retry
  - dry-run 既定 / `--sheet` / `--limit`
- 実施結果: **全 440 件を確認 → Active 0 / 判定不能 0 (全て Completed または ErrorCode 17)**。
  **HIGH 117 行 / LOW 323 行を退避後に削除**。
  再スキャンで **eBay に存在しない itemID の行は HIGH・LOW とも 0 件**。
  backup タブ: `deleted_HIGH_20260812_115247` (117) + `deleted_HIGH_20260812_121119` (55) /
  `deleted_LOW_20260812_122211` (323)。
- 効果: 巡回対象が **LOW 835→512 行 (-39%) / HIGH 1624→1507 行**。無駄な仕入元アクセスが減る。
- ★process: 初回実行は **1 行ずつ削除して Sheets の書込上限 (60/分) に当たり 429 で中断**した。
  バックアップは削除**前**に完了していたためデータ損失はゼロ。batch 化して再開し完走。
  「大量書込は必ず batch に畳む」を今後の既定にする。

### ③ エラー傾向の点検 (user「最近エラーが」)
- 直近 8 日のエラー率は **0.07〜0.15%** で横ばい (例 08-11: 18 / 12,210 行)。全 cycle success、
  `持続エラー 0` / `pending 0` / `action_required 0`。
- 内訳は `scraper returned None (fail-closed)` / `uncertain` / driver への `ReadTimeout` =
  **判定不能なら何もしない**設計どおりの挙動で、誤取下げ・誤復活には繋がらない。
- 唯一の常連が上記 row522 (×47) で、これは ② の削除で解消。
